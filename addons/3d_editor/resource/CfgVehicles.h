class CfgVehicles {

class Strategic;
class NonStrategic;
class SportsGrounds_base_F;
class LuggageHeap_base_F;
class Stall_base_F;
class Leaflet_base_F;
class Camping_base_F;
class Poster_base_F;
class BagBunker_base_F;
class HBarrier_base_F;
class Helipad_base_F;
class Graffiti_base_F;
class VR_Helper_base_F;
class Helper_Base_F;
class Shelter_base_F;
class CamoNet_INDP_open_F;
class BASE_WarfareBFieldhHospital;
class Small_items_NoInteractive;
class Thing_NoInteractive_EP1;
class Military_Item_NoInteractive;
class Land_VASICore;
class Items_Base_F;
class Thing;
class thingX;
class ThingEffect;
class Land_File1_F;
class Fortress1;
class ArrowDesk_L_F;
class Infostand_base_F;
class Constructions_base_F;
class FootBridge_Base_ACR;
class PaletaBase;
class Furniture_base_F;
class Market_base_F;
class RowBoats_base_F;
class IND_Box_Base;
class EAST_Box_Base;
class Misc_thing;
class Misc_thing_NoInteractive;
class NATO_Box_Base;
class Cargo_base_F;
class House_F;
class House_Small_f;
class Lamps_base_F;
class House;
class PowerLines_base_F;
class PowerLines_Small_base_F;
class PlayGround_base_F;
class Wreck_Base;
class Wreck_Base_F;
class UWreck_base_F;
class Small_items;
class SportItems_base_F;
class FoldChair;
class Helper_Base_EP1;
class BarrelBase;
class Dead_base_F;
class House_EP1;
class Land_i_Shop_01_V1_F;
class Cargo_House_base_F;
class Cargo_Tower_base_F;
class Cargo_HQ_base_F;
class Land_Wall_Gate_Ind1_R;
class Piers_base_F;
class VR_CoverObject_base_F;
class VR_Block_base_F;
class StaticCannon_Preview;
class WarfareBBaseStructure;
class Cargo_Patrol_base_F;
class Cemetery_base_F;
class PowerLines_Wires_base_F;
class Garbage_base_F;
class IndPipe_Small_base;
class IndPipe_Small_base_F;
class IndPipe_base_F;

//----------------------------------------------
//				Вещи
//----------------------------------------------
class Land_LuggageHeap_05_F : LuggageHeap_base_F {
    textPlural = "Вещи";
};
class Land_LuggageHeap_04_F : LuggageHeap_base_F {
    textPlural = "Вещи";
};
class Land_LuggageHeap_03_F : LuggageHeap_base_F {
    textPlural = "Вещи";
};
class Land_LuggageHeap_02_F : LuggageHeap_base_F {
    textPlural = "Вещи";
};
class Land_LuggageHeap_01_F : LuggageHeap_base_F {
    textPlural = "Вещи";
};
class Microphone1_ep1 : Small_items_NoInteractive {
    textPlural = "Вещи";
    displayName = "Микрофон";
};
class Microphone2_ep1 : Microphone1_ep1 {
    textPlural = "Вещи";
    displayName = "Микрофон (оранжевый)";
};
class Microphone3_ep1 : Microphone1_ep1 {
    textPlural = "Вещи";
    displayName = "Микрофон AAN";
};
class Misc_Backpackheap : Military_Item_NoInteractive {
    textPlural = "Вещи";
    displayName = "Рюкзаки (Куча)";
};
class Misc_Videoprojektor : Small_items_NoInteractive {
    textPlural = "Вещи";
    displayName = "Проектор";
};
class Misc_Videoprojektor_platno : Small_items_NoInteractive {
    textPlural = "Вещи";
    displayName = "Проектор (экран)";
};
class Misc_Wall_lamp : Small_items_NoInteractive {
    textPlural = "Вещи";
    displayName = "Лампа";
};
class Land_CarBattery_01_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CarBattery_02_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_SurvivalRadio_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Antibiotic_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_ButaneCanister_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_GasCanister_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Can_Rusty_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Can_V2_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Can_V1_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Battery_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Bandage_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Notepad_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Grinder_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PainKillers_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_ButaneTorch_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_VitaminBottle_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_BottlePlastic_V2_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Wrench_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_GasCooker_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_ClutterCutter_large_F : Thing {
    textPlural = "Вещи";
};
class Skeet_Clay_F : Items_base_F {
    textPlural = "Вещи";
};
class Skeet_Clay_NoPop_F : Skeet_Clay_F {
    textPlural = "Вещи";
};
class Land_DisinfectantSpray_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Defibrillator_F : Items_base_F {
    textPlural = "Вещи";
};
class Intel_File1_F : Land_File1_F {
    textPlural = "Вещи";
};
class Land_Document_01_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Can_Dented_F : Items_base_F {
    textPlural = "Вещи";	
};
class Land_Can_V3_F : Items_base_F {
    textPlural = "Вещи";	
};
class Land_TinContainer_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CanisterOil_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CanisterPlastic_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CanisterFuel_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PensAndPencils_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PencilYellow_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PencilGreen_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PencilRed_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_PencilBlue_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Map_altis_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Map_stratis_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Map_unfolded_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Map_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_DuctTape_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Compass_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CanOpener_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_CerealsBox_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Matches_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Shovel_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Magazine_rifle_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_MetalWire_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_MobilePhone_smart_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_MobilePhone_old_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Hammer_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_MultiMeter_F : Items_base_F {
    textPlural = "Вещи";
};
class FxCartridge_Small : ThingEffect {
    textPlural = "Вещи";
};
class FxCartridge_9mm : FxCartridge_Small {
    textPlural = "Вещи";
};
class Land_Laptop_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Laptop_unfolded_F : Items_base_F {
    textPlural = "Вещи";
};
class Land_Laptop_device_F : Land_Laptop_unfolded_F {
    textPlural = "Вещи";
};
class Land_FireExtinguisher_F : Items_base_F {
    textPlural = "Вещи";
};
class Bomb : Strategic {
    textPlural = "Вещи";
};
class Notebook : Small_items {
    textPlural = "Вещи";
};


//----------------------------------------------
//				Картинг
//----------------------------------------------
class Land_TyreBarrier_01_F : SportsGrounds_base_F {
    textPlural = "Картинг";
};
class Land_PlasticBarrier_01_F : SportsGrounds_base_F {
    textPlural = "Картинг";
};
class Land_PlasticBarrier_02_F : SportsGrounds_base_F {
    textPlural = "Картинг";
};
class TyreBarrier_01_white_F : Land_TyreBarrier_01_F {
    textPlural = "Картинг";
};
class TyreBarrier_01_black_F : Land_TyreBarrier_01_F {
    textPlural = "Картинг";
};
class Land_TyreBarrier_01_line_x4_F : Land_TyreBarrier_01_F {
    textPlural = "Картинг";
};
class Land_TyreBarrier_01_line_x6_F : Land_TyreBarrier_01_F {
    textPlural = "Картинг";
};
class PlasticBarrier_01_white_F : Land_PlasticBarrier_01_F {
    textPlural = "Картинг";
};
class PlasticBarrier_01_red_F : Land_PlasticBarrier_01_F {
    textPlural = "Картинг";
};
class Land_PlasticBarrier_01_line_x2_F : Land_PlasticBarrier_01_F {
    textPlural = "Картинг";
};
class Land_PlasticBarrier_01_line_x4_F : Land_PlasticBarrier_01_F {
    textPlural = "Картинг";
};
class Land_PlasticBarrier_01_line_x6_F : Land_PlasticBarrier_01_F {
    textPlural = "Картинг";
};
class PlasticBarrier_02_yellow_F : Land_PlasticBarrier_02_F {
    textPlural = "Картинг";
};
class PlasticBarrier_02_grey_F : Land_PlasticBarrier_02_F {
    textPlural = "Картинг";
};
class Land_WinnersPodium_01_F : SportsGrounds_base_F {
    textPlural = "Картинг";
};
class Land_FinishGate_01_narrow_F : SportsGrounds_base_F {
    textPlural = "Картинг";
};
class Land_FinishGate_01_wide_F : Land_FinishGate_01_narrow_F {
    textPlural = "Картинг";
};
class Land_PartyTent_01_F : Camping_base_F {
    textPlural = "Картинг";
};
class Land_KartSteertingWheel_01_F : SportItems_base_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_white_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_yellow_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_green_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_red_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_orange_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};
class KartSteertingWheel_01_blue_F : Land_KartSteertingWheel_01_F {
    textPlural = "Картинг";
};




//----------------------------------------------
//				Спорт
//----------------------------------------------
class Land_BC_Court_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_BC_Basket_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_Goal_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_Tribune_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_GymBench_01_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_GymRack_03_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_GymRack_01_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_GymRack_02_F : SportsGrounds_base_F {
    textPlural = "Спорт";
};
class Land_Trophy_01_gold_F : SportItems_base_F {
    textPlural = "Спорт";
};
class Land_Trophy_01_bronze_F : Land_Trophy_01_gold_F {
    textPlural = "Спорт";
};
class Land_Trophy_01_silver_F : Land_Trophy_01_gold_F {
    textPlural = "Спорт";
};
class SkeetMachine : Small_items {
    textPlural = "Спорт";
};
class SkeetDisk : Small_items {
    textPlural = "Спорт";
};
class Land_Stadium_p1_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p2_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p3_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p4_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p5_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p6_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p7_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p8_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_p9_F : House_F {
    textPlural = "Спорт";
};
class Land_Stadium_score_F : House_F {
    textPlural = "Спорт";
};
class Bleacher_EP1 : House {
    textPlural = "Спорт";
    displayName = "Трибуна";
};



//----------------------------------------------
//				Бетонные сооружения
//----------------------------------------------
class Land_RampConcrete_F : NonStrategic {
    textPlural = "Бетонные сооружения";
};
class RampConcrete : NonStrategic {
    textPlural = "Бетонные сооружения";
};
class Hhedgehog_concrete : Strategic {
    textPlural = "Бетонные сооружения";
    displayName = "Противотанковые";
};
class Hhedgehog_concreteBig : Hhedgehog_concrete {
    textPlural = "Бетонные сооружения";
    displayName = "Противотанковые (большие)";
};
class Land_CncBlock : NonStrategic {
    textPlural = "Бетонные сооружения";
    displayName = "Заграждение (низкое, ржавое)";
};
class Land_CncBlock_D : NonStrategic {
    textPlural = "Бетонные сооружения";
    displayName = "Заграждение (низкое, ржавое, сломонное)";
};
class Land_CncBlock_Stripes : NonStrategic {
    textPlural = "Бетонные сооружения";
    displayName = "Заграждение (низкое, окрашенное)";
};
class Land_RampConcreteHigh_F : House_F {
    textPlural = "Бетонные сооружения";
};
class Land_CncShelter_F : House_Small_F {
    textPlural = "Бетонные сооружения";
};
class BlockConcrete_F : House_F {
    textPlural = "Бетонные сооружения";
};


//----------------------------------------------
//				Бетонные сооружения
//----------------------------------------------
class HumpsDirt : NonStrategic {
    textPlural = "Природа";
};
class Land_Timbers_F : NonStrategic {
    textPlural = "Природа";
};
class Dirthump_4_F : NonStrategic {
    textPlural = "Природа";
};
class Land_WoodPile_large_F : NonStrategic {
    textPlural = "Природа";
};
class Land_WoodPile_F : NonStrategic {
    textPlural = "Природа";
};
class CraterLong : NonStrategic {
    textPlural = "Природа";
    displayName = "Кратер";
};
class CraterLong_small : CraterLong {
    textPlural = "Природа";
    displayName = "Кратер (маленький)";
};
class Land_WoodenLog_F : thingX {
    textPlural = "Природа";
};
class FxExploGround1 : ThingEffect {
    textPlural = "Природа";
};
class FxExploGround2 : ThingEffect {
    textPlural = "Природа";
};
class TransparentWall : ThingEffect {
    textPlural = "Природа";
};
class Land_Sea_Wall_F : Piers_base_F {
    textPlural = "Природа";
};
class Dirthump_1_F : House_F {
    textPlural = "Природа";
};
class Dirthump_2_F : Dirthump_1_F {
    textPlural = "Природа";
};
class Dirthump_3_F : Dirthump_1_F {
    textPlural = "Природа";
};
class Land_Ind_Timbers : House {
    textPlural = "Природа";
    displayName = "";
};
class Land_Dirthump01 : House {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Dirthump01_EP1 : Land_Dirthump01 {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Dirthump02 : House {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Dirthump02_EP1 : Land_Dirthump02 {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Dirthump03 : House {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Dirthump03_EP1 : Land_Dirthump03 {
    textPlural = "Природа";
    displayName = "Грязь";
};
class Land_Misc_Coltan_Heap_EP1 : House_EP1 {
    textPlural = "Природа";
    displayName = "Куча песка";
};
class Dirtmount_EP1 : House {
    textPlural = "Природа";
    displayName = "Грязь";
};





//----------------------------------------------
//              Лагерь
//----------------------------------------------
class Camp_base;
class Land_Fire : House {
    textPlural = "Лагерь";
};
class Land_Fire_burning : Land_Fire {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_F : Camping_base_F {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_brown_F : Land_Sleeping_bag_F {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_blue_F : Land_Sleeping_bag_F {
    textPlural = "Лагерь";
};
class Respawn_Sleeping_bag_F : Land_Sleeping_bag_F {
    textPlural = "Лагерь";
};
class Respawn_Sleeping_bag_brown_F : Land_Sleeping_bag_brown_F {
    textPlural = "Лагерь";
};
class Land_TentDome_F : Camping_base_F {
    textPlural = "Лагерь";
};
class Respawn_TentDome_F : Land_TentDome_F {
    textPlural = "Лагерь";
};
class Respawn_Sleeping_bag_blue_F : Land_Sleeping_bag_blue_F {
    textPlural = "Лагерь";
};
class Land_TentA_F : Camping_base_F {
    textPlural = "Лагерь";
};
class Camp : Camp_base {
    textPlural = "Лагерь";
};
class Camp_EP1 : Camp {
    textPlural = "Лагерь";
};
class ACamp : Camp_base {
    textPlural = "Лагерь";
    displayName = "Палатка";
};
class CampEast : Camp_base {
    textPlural = "Лагерь";
    displayName = "Палатка (большая)";
};
class Land_Campfire : Land_Fire {
    textPlural = "Лагерь";
};
class Land_Campfire_F : House_F {
    textPlural = "Лагерь";
};
class Campfire_burning_F : Land_Campfire_F {
    textPlural = "Лагерь";
};
class Land_Campfire_burning : Land_Campfire {
    textPlural = "Лагерь";
};
class Land_Pillow_F : thingX {
    textPlural = "Лагерь";
};
class Land_Pillow_camouflage_F : Land_Pillow_F {
    textPlural = "Лагерь";
};
class Land_Pillow_grey_F : Land_Pillow_F {
    textPlural = "Лагерь";
};
class Land_Pillow_old_F : Land_Pillow_F {
    textPlural = "Лагерь";
};
class Land_Camping_Light_off_F : thingX {
    textPlural = "Лагерь";
};
class Land_CampingTable_small_F : thingX {
    textPlural = "Лагерь";
};
class Land_CampingTable_F : thingX {
    textPlural = "Лагерь";
};
class Land_CampingChair_V1_F : thingX {
    textPlural = "Лагерь";
};
class Land_CampingChair_V2_F : thingX {
    textPlural = "Лагерь";
};
class Land_Ground_sheet_folded_F : thingX {
    textPlural = "Лагерь";
};
class Land_Ground_sheet_folded_yellow_F : Land_Ground_sheet_folded_F {
    textPlural = "Лагерь";
};
class Land_Ground_sheet_folded_blue_F : Land_Ground_sheet_folded_F {
    textPlural = "Лагерь";
};
class Land_Ground_sheet_folded_khaki_F : Land_Ground_sheet_folded_F {
    textPlural = "Лагерь";
};
class Land_Ground_sheet_folded_OPFOR_F : Land_Ground_sheet_folded_F {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_folded_F : thingX {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_brown_folded_F : Land_Sleeping_bag_folded_F {
    textPlural = "Лагерь";
};
class Land_Sleeping_bag_blue_folded_F : Land_Sleeping_bag_folded_F {
    textPlural = "Лагерь";
};
class Land_FirePlace_F : House_F {
    textPlural = "Лагерь";
};
class FirePlace_burning_F : Land_FirePlace_F {
    textPlural = "Лагерь";
};
class Land_tent_east : House {
    textPlural = "Лагерь";
    displayName = "Тент";
};





//----------------------------------------------
//				Обозначения на поверхности
//----------------------------------------------
class Land_HelipadCivil_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_HelipadSquare_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_HelipadCircle_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_HelipadEmpty_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_HelipadRescue_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_Graffiti_01_F : Graffiti_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_Graffiti_04_F : Graffiti_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_Graffiti_03_F : Graffiti_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_Graffiti_02_F : Graffiti_base_F {
    textPlural = "Обозначения на поверхности";
};
class Land_Graffiti_05_F : Graffiti_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_GroundIcon_01_F : VR_Helper_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_square_1x1_grey_F : VR_Helper_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_square_1x1_yellow_F : VR_Area_01_square_1x1_grey_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_square_2x2_yellow_F : VR_Area_01_square_1x1_yellow_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_square_4x4_yellow_F : VR_Area_01_square_1x1_yellow_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_square_4x4_grey_F : VR_Area_01_square_1x1_grey_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_circle_4_grey_F : VR_Helper_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Area_01_circle_4_yellow_F : VR_Area_01_circle_4_grey_F {
    textPlural = "Обозначения на поверхности";
};
class Land_JumpTarget_F : Helipad_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Sector_01_60deg_50_grey_F : VR_Helper_base_F {
    textPlural = "Обозначения на поверхности";
};
class VR_Sector_01_60deg_50_red_F : VR_Sector_01_60deg_50_grey_F {
    textPlural = "Обозначения на поверхности";
};
class Oil_Spill_F : Helper_Base_F {
    textPlural = "Обозначения на поверхности";
};
class HeliH : NonStrategic {
    displayName = "Вертолетная площадка";
    textPlural = "Обозначения на поверхности";
};
class HeliHCivil : HeliH {
    displayName = "Вертолетная площадка (гражданская)";
    textPlural = "Обозначения на поверхности";
};
class HeliHRescue : HeliH {
    displayName = "Вертолетная площадка (мед)";
    textPlural = "Обозначения на поверхности";
};
class Sr_border : NonStrategic {
    displayName = "Граница (желтая)";
    textPlural = "Обозначения на поверхности";
};


//----------------------------------------------
//				Обозначения на поверхности
//----------------------------------------------
class VR_3DSelector_01_default_F : VR_Helper_base_F {
    textPlural = "Виртуальная реальность";
};
class VR_3DSelector_01_exit_F : VR_3DSelector_01_default_F {
    textPlural = "Виртуальная реальность";
};
class VR_3DSelector_01_incomplete_F : VR_3DSelector_01_default_F {
    textPlural = "Виртуальная реальность";
};
class VR_3DSelector_01_complete_F : VR_3DSelector_01_default_F {
    textPlural = "Виртуальная реальность";
};
class VR_Billboard_01_F : VR_Helper_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_CoverObject_01_standHigh_F : VR_CoverObject_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_CoverObject_01_kneelHigh_F : VR_CoverObject_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_CoverObject_01_stand_F : VR_CoverObject_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_CoverObject_01_kneel_F : VR_CoverObject_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_CoverObject_01_kneelLow_F : VR_CoverObject_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_Block_04_F : VR_Block_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_Block_05_F : VR_Block_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_Block_03_F : VR_Block_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_Block_01_F : VR_Block_base_F {
    textPlural = "Виртуальная реальность";
};
class Land_VR_Block_02_F : VR_Block_base_F {
    textPlural = "Виртуальная реальность";
};



//----------------------------------------------
//				Свет
//----------------------------------------------
class Land_Runway_PAPI : Land_VASICore {
    textPlural = "Свет";
};
class Land_Runway_PAPI_2 : Land_Runway_PAPI {
    textPlural = "Свет";
};
class Land_Runway_PAPI_3 : Land_Runway_PAPI {
    textPlural = "Свет";
};
class Land_Runway_PAPI_4 : Land_Runway_PAPI {
    textPlural = "Свет";
};
class Land_LampHarbour_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampHarbour_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampHalogen_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampHalogen_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampDecor_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampDecor_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampSolar_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampSolar_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampStadium_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampShabby_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampShabby_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_NavigLight : House {
    textPlural = "Свет";
};
class Land_NavigLight_3_F : Land_NavigLight {
    textPlural = "Свет";
};
class Land_FloodLight_F : Items_base_F {
    textPlural = "Свет";
};
class Land_LampStreet_small_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampStreet_small_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampStreet_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_LampStreet_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_runway_edgelight : House {
    textPlural = "Свет";
};
class Land_runway_edgelight_blue_F : Land_runway_edgelight {
    textPlural = "Свет";
};
class Land_PortableLight_single_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_PortableLight_double_F : Land_PortableLight_single_F {
    textPlural = "Свет";
};
class Land_LampAirport_off_F : Lamps_base_F {
    textPlural = "Свет";
};
class Land_Stoplight01 : House {
    textPlural = "Свет";
    displayName = "Светофор 1";
};
class Land_Stoplight02 : Land_Stoplight01 {
    textPlural = "Свет";
    displayName = "Светофор 2";
};
class Land_LampAirport_F : Land_LampAirport_off_F {
    textPlural = "Свет";
};



//----------------------------------------------
//				ЛЭП
//----------------------------------------------
class Land_Loudspeakers_F : NonStrategic {
    textPlural = "ЛЭП";
};
class Land_HighVoltageTower_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerPoleWooden_L_off_F : PowerLines_Small_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerPoleWooden_L_F : PowerLines_Small_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerPoleWooden_small_F : PowerLines_Small_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerPoleWooden_F : PowerLines_Small_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerPoleConcrete_F : PowerLines_Small_base_F {
    textPlural = "ЛЭП";
};
class Land_Com_tower_ep1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "Башня (сотовая)";
};
class Land_HighVoltageTower_large_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_HighVoltageTower_largeCorner_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_HighVoltageTower_dam_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_HighVoltageColumn_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_PowLines_Transformer_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_TTowerSmall_2_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_TTowerSmall_1_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_PowerLine_distributor_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerLine_part_F : PowerLines_base_F {
    textPlural = "ЛЭП";
};
class Land_PowerGenerator_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_Communication_F : House_F {
    textPlural = "ЛЭП";
};
class Land_TTowerBig_1_F : House_F {
    textPlural = "ЛЭП";
};
class Land_TTowerBig_2_F : House_F {
    textPlural = "ЛЭП";
};
class Land_trafostanica_velka_draty : House {
    textPlural = "ЛЭП";
};
class Land_trafostanica_velka : House {
    textPlural = "ЛЭП";
};
class Land_trafostanica_mala : House {
    textPlural = "ЛЭП";
};
class Land_sloup_vn_drat : House {
    textPlural = "ЛЭП";
    displayName = "ЛЭП 3";
};
class Land_sloup_vn_dratZ : House {
    textPlural = "ЛЭП";
    displayName = "ЛЭП";
};
class Land_sloup_vn_drat_d : Land_sloup_vn_drat {
    textPlural = "ЛЭП";
    displayName = "ЛЭП 2";
};
class Land_sloup_vn : House {
    textPlural = "ЛЭП";
};
class Land_Communication_anchor_F : House_F {
    textPlural = "ЛЭП";
};
class Land_SolarPanel_3_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_spp_Panel_Broken_F : House_F {
    textPlural = "ЛЭП";
};
class Land_SolarPanel_1_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_SolarPanel_2_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_spp_Mirror_Broken_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_spp_Mirror_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_spp_Transformer_F : House_Small_f {
    textPlural = "ЛЭП";
};
class Land_Vysilac_FM : House {
    textPlural = "ЛЭП";
};
class Land_vysilac_FM2 : Land_Vysilac_FM {
    textPlural = "ЛЭП";
};
class Land_radar : Land_vysilac_FM2 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_telek1 : House {
    textPlural = "ЛЭП";
    displayName = "Телевышка";
};
class Land_radar_EP1 : Land_radar {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireBig_direct_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireBig_direct_short_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireBig_end_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireBig_left_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireBig_right_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireSmall_damaged_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireSmall_direct_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireSmall_Left_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowerWireSmall_Right_F : PowerLines_Wires_base_F {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLine_wire_A_left_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLine_wire_A_right_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLine_wire_AB_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLine_wire_BB_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLineA_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLineB_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLines_Conc1_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLines_Conc2_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLines_Conc2A_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_PowLines_Transformer1_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_Misc_Cable_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_Misc_Cable_Rugs1_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_Misc_Cable_V_EP1 : House_EP1 {
    textPlural = "ЛЭП";
    displayName = "";
};
class Land_spp_Panel_F : House_F {
    textPlural = "ЛЭП";
};


//----------------------------------------------
//              ЖД
//----------------------------------------------
class Land_wagon_tanker : House {
    textPlural = "ЖД";
    displayName = "";
};
class Land_wagon_flat : House {
    textPlural = "ЖД";
    displayName = "";
};
class Land_wagon_box : House {
    textPlural = "ЖД";
    displayName = "";
};
class Land_Rail_House_01 : House {
    textPlural = "ЖД";
    displayName = "Стация";
};
class Land_Rail_Semafor : House {
    textPlural = "ЖД";
    displayName = "Семафор";
};
class Land_rail_station_big : House {
    textPlural = "ЖД";
    displayName = "Вокзал";
};
class Land_Rail_Zavora : House {
    textPlural = "ЖД";
};
class Land_rails_bridge_40 : House {
    textPlural = "ЖД";
    displayName = "Мост";
};



//----------------------------------------------
//				Дорога
//----------------------------------------------
class RoadBarrier_light : NonStrategic {
    textPlural = "Дорога";
};
class RoadBarrier_long : RoadBarrier_light {
    textPlural = "Дорога";
};
class RoadCone_F : thingX {
    textPlural = "Дорога";
};
class RoadBarrier_F : RoadCone_F {
    textPlural = "Дорога";
};
class ArrowMarker_R_F : ArrowDesk_L_F {
    textPlural = "Дорога";
};
class ArrowMarker_L_F : ArrowMarker_R_F {
    textPlural = "Дорога";
};
class Land_RedWhitePole_F : thingX {
    textPlural = "Дорога";
};
class Land_RedWhiteBarrier : House {
    textPlural = "Дорога";
};

//----------------------------------------------
//				Знаки
//----------------------------------------------
class Land_Noticeboard_F : NonStrategic {
    textPlural = "Знаки";
};
class Land_Sign_Mines_F : NonStrategic {
    textPlural = "Знаки";
};
class Land_Sign_WarningMilitaryVehicles_F : Land_Sign_Mines_F {
    textPlural = "Знаки";
};
class Land_Sign_WarningMilitaryArea_F : Land_Sign_Mines_F {
    textPlural = "Знаки";
};
class Land_Sign_WarningMilAreaSmall_F : Land_Sign_Mines_F {
    textPlural = "Знаки";
};
class Land_Sign_WarningUnexplodedAmmo_F : Land_Sign_Mines_F {
    textPlural = "Знаки";
};
class Sign_Direction_F : NonStrategic {
    textPlural = "Знаки";
};
class FlagSmall_F : NonStrategic {
    textPlural = "Знаки";
};
class FlagCarrierSmall : NonStrategic {
    textPlural = "Знаки";
    displayName = "Железный флажок (желтый)";
};
class WarfareBunkerSign : NonStrategic {
    textPlural = "Знаки";
    displayName = "USA";
};
class SignM_FARP_Winchester_EP1 : WarfareBunkerSign {
    textPlural = "Знаки";
    displayName = "Winchester";
};
class SignM_FOB_Blanik_EP1 : WarfareBunkerSign {
    textPlural = "Знаки";
    displayName = "Army of the Czech republic";
};
class SignM_FOB_Prostejov_EP1 : WarfareBunkerSign {
    textPlural = "Знаки";
    displayName = "Army of the Czech republic 2";
};
class SignM_FOB_Revolve_EP1 : WarfareBunkerSign {
    textPlural = "Знаки";
    displayName = "For revolver";
};
class SignM_UN_Base_EP1 : WarfareBunkerSign {
    textPlural = "Знаки";
    displayName = "UNO";
};
class Land_InfoStand_V1_F : Infostand_base_F {
    textPlural = "Знаки";
};
class Land_InfoStand_V2_F : Infostand_base_F {
    textPlural = "Знаки";
};
class Sign_1L_Border : Thing {
    textPlural = "Знаки";
};
class Sign_DangerMines_ACR : Sign_1L_Border {
    textPlural = "Знаки";
};
class Sign_1L_Firstaid : Sign_1L_Border {
    textPlural = "Знаки";
};
class Sign_1L_Firstaid_EP1 : Sign_1L_Firstaid {
    textPlural = "Знаки";
};
class Sign_1L_Noentry : Sign_1L_Border {
    textPlural = "Знаки";
};
class Sign_1L_Noentry_EP1 : Sign_1L_Noentry {
    textPlural = "Знаки";
};
class Sign_Danger : Thing {
    textPlural = "Знаки";
};
class Sign_Checkpoint : Sign_Danger {
    textPlural = "Знаки";
};
class Sign_Checkpoint_EP1 : Sign_Checkpoint {
    textPlural = "Знаки";
};
class Sign_Checkpoint_TK_EP1 : Sign_Checkpoint {
    textPlural = "Знаки";
};
class Sign_Checkpoint_US_EP1 : Sign_Checkpoint {
    textPlural = "Знаки";
};
class Sign_MP_blu : Sign_Danger {
    textPlural = "Знаки";
};
class Sign_MP_blu_EP1 : Sign_MP_blu {
    textPlural = "Знаки";
};
class Sign_MP_ind : Sign_Danger {
    textPlural = "Знаки";
};
class Sign_MP_ind_EP1 : Sign_MP_ind {
    textPlural = "Знаки";
};
class Sign_MP_op : Sign_Danger {
    textPlural = "Знаки";
};
class Sign_MP_op_EP1 : Sign_MP_op {
    textPlural = "Знаки";
};
class Sign_Stop : Sign_Checkpoint {
    textPlural = "Знаки";
};
class Sign_tape_redwhite : Sign_Danger {
    textPlural = "Знаки";
};





//----------------------------------------------
//				Укрепления
//----------------------------------------------
class BagFence_base_F;
class Land_BagFence_Long_F : BagFence_base_F {
    textPlural = "Укрепления";
};
class Land_BagFence_End_F : BagFence_base_F {
    textPlural = "Укрепления";
};
class Land_BagFence_Short_F : BagFence_base_F {
    textPlural = "Укрепления";
};
class Land_BagFence_Round_F : BagFence_base_F {
    textPlural = "Укрепления";
};
class Land_BagFence_Corner_F : BagFence_base_F {
    textPlural = "Укрепления";
};
class Land_Razorwire_F : NonStrategic {
    textPlural = "Укрепления";
};
class Land_HBarrier_1_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrierBig_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrier_Big_F : Land_HBarrierBig_F {
    textPlural = "Укрепления";
};
class Land_HBarrier_3_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrier_5_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrierWall_corridor_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrierWall6_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrierWall4_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Land_HBarrierWall_corner_F : HBarrier_base_F {
    textPlural = "Укрепления";
};
class Fort_RazorWire : NonStrategic {
    textPlural = "Укрепления";
    displayName = "Колючая проволока";
};
class Land_HBarrier_large : NonStrategic {
    textPlural = "Укрепления";
    displayName = "HBarier Large";
};
class Land_BagFenceCorner : Land_HBarrier_large {
    textPlural = "Укрепления";
    displayName = "Мешки (зеленые, угол)";
};
class Land_BagFenceEnd : Land_BagFenceCorner {
    textPlural = "Укрепления";
    displayName = "Мешки (зеленые, край)";
};
class Land_BagFenceLong : Land_BagFenceCorner {
    textPlural = "Укрепления";
    displayName = "Мешки (зеленые, длинные)";
};
class Land_BagFenceRound : Land_BagFenceCorner {
    textPlural = "Укрепления";
    displayName = "Мешки (зеленые, полукруг)";
};
class Land_BagFenceShort : Land_BagFenceCorner {
    textPlural = "Укрепления";
    displayName = "Мешки (зеленые, короткие)";
};
class Land_HBarrier1 : Land_HBarrier_large {
    textPlural = "Укрепления";
    displayName = "HBarier 1";
};
class Land_HBarrier3 : Land_HBarrier_large {
    textPlural = "Укрепления";
    displayName = "HBarier 3";
};
class Land_HBarrier5 : Land_HBarrier_large {
    textPlural = "Укрепления";
    displayName = "HBarier 5";
};
class Fort_EnvelopeBig : House {
    textPlural = "Укрепления";
    displayName = "Окоп (длинный, зеленый)";
};
class Fort_EnvelopeBig_EP1 : Fort_EnvelopeBig {
    textPlural = "Укрепления";
    displayName = "Окоп (длинный, серый)";
};
class Fort_EnvelopeSmall : House {
    textPlural = "Укрепления";
    displayName = "Окоп (личный, зеленый)";
};
class Fort_EnvelopeSmall_EP1 : Fort_EnvelopeSmall {
    textPlural = "Укрепления";
    displayName = "Окоп (личный, серый)";

};
class Base_WarfareBBarrier10x : Base_WarfareBBarrier5x {
    textPlural = "Укрепления";
    displayName = "HBarrier2 10";
};
class Base_WarfareBBarrier10xTall : Base_WarfareBBarrier5x {
    textPlural = "Укрепления";
    displayName = "HBarrier2 10x2";
};
class Base_WarfareBBarrier5x : House {
    textPlural = "Укрепления";
    displayName = "HBarrier2 5";
};






//----------------------------------------------
//				Каммуфляж
//----------------------------------------------
class CamoNet_BLUFOR_F : Shelter_base_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (зеленая)";
};
class CamoNet_INDP_F : CamoNet_BLUFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (лес)";
};
class CamoNet_OPFOR_F : CamoNet_BLUFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (пустыня)";
};
class CamoNet_BLUFOR_big_F : CamoNet_BLUFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфлированный брезент для техники (зеленая)";

};
class CamoNet_INDP_big_F : CamoNet_INDP_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфлированный брезент для техники (лес)";
};
class CamoNet_OPFOR_big_F : CamoNet_OPFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфлированный брезент для техники (пустыня)";
};
class CamoNet_BLUFOR_open_F : CamoNet_BLUFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (открытая, зеленая)";
};
class CamoNet_INDP_open_Curator_F : CamoNet_INDP_open_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (открытая, лес)";
};
class CamoNet_OPFOR_open_F : CamoNet_OPFOR_F {
    textPlural = "Каммуфляж";
    displayName = "Камуфляжная сетка (открытая, пустыня)";
};




//----------------------------------------------
//				Город (антураж)
//----------------------------------------------
class Land_StallWater_F : Stall_base_F {
    textPlural = "Город (антураж)";
    displayName = "Колодец";
};
class Land_TreeBin_F : NonStrategic {
    textPlural = "Город (антураж)";
    displayName = "Колодец (засохший)";
};
class Land_Cages_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_Leaflet_03_F : Leaflet_base_F {
    textPlural = "Город (антураж)";
};
class Land_Leaflet_04_F : Leaflet_base_F {
    textPlural = "Город (антураж)";
};
class Land_Leaflet_01_F : Leaflet_base_F {
    textPlural = "Город (антураж)";
};
class Land_Leaflet_02_F : Leaflet_base_F {
    textPlural = "Город (антураж)";
};
class Land_Sacks_heap_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_Sacks_goods_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_Sack_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_01_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_02_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_03_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_04_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_05_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Poster_06_F : Poster_base_F {
    textPlural = "Город (антураж)";
};
class Land_Pallets_F : NonStrategic {
    textPlural = "Город (антураж)";
};
class Land_Billboard_F : NonStrategic {
    textPlural = "Город (антураж)";
};
class Land_Water_source_F : NonStrategic {
    textPlural = "Город (антураж)";
};
class Land_MarketShelter_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_CratesShabby_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_CratesWooden_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Land_CratesPlastic_F : Stall_base_F {
    textPlural = "Город (антураж)";
};
class Fort_Barricade : Strategic {
    textPlural = "Город (антураж)";
    displayName = "Барикада";
};
class Land_Crates_EP1 : Thing_NoInteractive_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Пластиковые коробки";
};
class Land_kolotoc : Strategic {
    textPlural = "Город (антураж)";
    displayName = "kolotoc";
};
class Land_Toilet : NonStrategic {
    textPlural = "Город (антураж)";
    displayName = "Туалет (желтый)";
};
class Loudspeaker : Small_items_NoInteractive {
    textPlural = "Город (антураж)";
    displayName = "Громкоговоритель";
};
class Paleta1 : PaletaBase {
    textPlural = "Город (антураж)";
    displayName = "Поддон";
};
class Paleta2 : PaletaBase {
    textPlural = "Город (антураж)";
    displayName = "Поддон x3";
};
class Land_GarbageContainer_closed_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_GarbageContainer_open_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_WaterTank_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_WaterBarrel_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_GarbageBarrel_01_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_Bucket_painted_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_Bucket_clean_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BucketNavy_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_Bucket_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_WorkStand_F : Constructions_base_F {
    textPlural = "Город (антураж)";
};
class Land_WaterCooler_01_new_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_WaterCooler_01_old_F : Land_WaterCooler_01_new_F {
    textPlural = "Город (антураж)";
};
class Land_WoodenTable_large_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_WoodenTable_small_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_WoodenBox_F : Constructions_base_F {
    textPlural = "Город (антураж)";
};
class Land_CashDesk_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_Basket_F : Market_base_F {
    textPlural = "Город (антураж)";
};
class Land_Ground_sheet_F : Camping_base_F {
    textPlural = "Город (антураж)";
};
class Land_CampingChair_V1_folded_F : Camping_base_F {
    textPlural = "Город (антураж)";
};
class Land_Ground_sheet_yellow_F : Land_Ground_sheet_F {
    textPlural = "Город (антураж)";
};
class Land_Ground_sheet_blue_F : Land_Ground_sheet_F {
    textPlural = "Город (антураж)";
};
class Land_Ground_sheet_khaki_F : Land_Ground_sheet_F {
    textPlural = "Город (антураж)";
};
class Land_Ground_sheet_OPFOR_F : Land_Ground_sheet_F {
    textPlural = "Город (антураж)";
};
class Land_Scaffolding_F : NonStrategic {
    textPlural = "Город (антураж)";
};
class Garbage_can : Strategic {
    textPlural = "Город (антураж)";
    displayName = "Мусорная корзина (металл)";
};
class Garbage_container : Strategic {
    textPlural = "Город (антураж)";
    displayName = "Мусорная корзина (металл, большая)";
};
class Land_RowBoat_V1_F : RowBoats_base_F {
    textPlural = "Город (антураж)";
};
class Land_RowBoat_V2_F : RowBoats_base_F {
    textPlural = "Город (антураж)";
};
class Land_RowBoat_V3_F : RowBoats_base_F {
    textPlural = "Город (антураж)";
};
class Land_Metal_rack_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_Metal_rack_Tall_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_Atm_02_F : House_Small_F {
    textPlural = "Город (антураж)";
};
class Land_Atm_01_F : House_Small_F {
    textPlural = "Город (антураж)";
};
class Land_cargo_addon02_V1_F : House_Small_F {
    textPlural = "Город (антураж)";
};
class Land_Slide_F : PlayGround_base_F {
    textPlural = "Город (антураж)";
};
class Land_TableDesk_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_FoodContainer_01_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelTrash_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelEmpty_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelEmpty_grey_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelWater_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelWater_grey_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelTrash_grey_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_BarrelSand_grey_F : Items_base_F {
    textPlural = "Город (антураж)";
};
class Land_PlasticBarrier_03_F : thingX {
    textPlural = "Город (антураж)";
};
class PlasticBarrier_03_orange_F : Land_PlasticBarrier_03_F {
    textPlural = "Город (антураж)";
};
class PlasticBarrier_03_blue_F : Land_PlasticBarrier_03_F {
    textPlural = "Город (антураж)";
};
class Land_Pallet_vertical_F : Constructions_base_F {
    textPlural = "Город (антураж)";
};
class Land_Pallet_F : Constructions_base_F {
    textPlural = "Город (антураж)";
};
class Land_FieldToilet_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_ShelvesWooden_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_ShelvesWooden_blue_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_ShelvesWooden_khaki_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_ShelvesMetal_F : Furniture_base_F {
    textPlural = "Город (антураж)";
};
class Land_Sink_F : thingX {
    textPlural = "Город (антураж)";
};
class Land_TablePlastic_01_F : thingX {
    textPlural = "Город (антураж)";
};
class SmallTable : Thing {
    textPlural = "Город (антураж)";
};
class SmallTV : Small_items {
    textPlural = "Город (антураж)";
};
class Suitcase : Small_items {
    textPlural = "Город (антураж)";
};
class WoodChair : FoldChair {
    textPlural = "Город (антураж)";
};
class Land_Coil_F : Constructions_base_F {
    textPlural = "Город (антураж)";
};
class ShootingRange_ACR : House {
    textPlural = "Город (антураж)";
};
class Land_PhoneBooth_02_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_PhoneBooth_01_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_dp_transformer_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_Bench_01_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_Bench_02_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_SlideCastle_F : PlayGround_base_F {
    textPlural = "Город (антураж)";
};
class Land_Swing_01_F : PlayGround_base_F {
    textPlural = "Город (антураж)";
};
class Land_Carousel_01_F : PlayGround_base_F {
    textPlural = "Город (антураж)";
};
class Land_MetalBarrel_empty_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class MetalBarrel_burning_F : Land_MetalBarrel_empty_F {
    textPlural = "Город (антураж)";
};
class Land_GarbagePallet_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_GarbageWashingMachine_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_GarbageBin_01_F : House_Small_f {
    textPlural = "Город (антураж)";
};
class Land_GarbageBags_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_Garbage_line_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_Garbage_square3_F : Garbage_base_F {
    textPlural = "Город (антураж)";};
class Land_Garbage_square5_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_Tyres_F : Garbage_base_F {
    textPlural = "Город (антураж)";
};
class Land_tires_EP1 : House_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Шины";
};
class Land_Misc_Garb_Heap_EP1 : House_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Мусорные пакеты";
};
class Misc_TyreHeap : Land_Fire {
    textPlural = "Город (антураж)";
    displayName = "Шины";
};
class Misc_TyreHeapEP1 : Misc_TyreHeap {
    textPlural = "Город (антураж)";
    displayName = "Шины";
};
class Land_Misc_Garb_3_EP1 : House_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Мусор";
};
class Land_Misc_Garb_4_EP1 : House_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Мусор (длинный)";
};
class Land_Misc_Garb_Square_EP1 : House_EP1 {
    textPlural = "Город (антураж)";
    displayName = "Мусор (много)";
};






//----------------------------------------------
//				Помощник
//----------------------------------------------
class Sign_Circle_F : Helper_Base_F {
    textPlural = "Помощник";
};
class CubeHelper : NonStrategic {
    textPlural = "Помощник";
};
class BASE_WarfareBFieldhHospitalPreview : BASE_WarfareBFieldhHospital {
    textPlural = "Помощник";
};
class US_WarfareBFieldhHospitalPreview_EP1 : BASE_WarfareBFieldhHospitalPreview {
    textPlural = "Помощник";
};
class USMC_WarfareBFieldhHospitalPreview : BASE_WarfareBFieldhHospitalPreview {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_Cyan_F : Sign_Arrow_Large_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_Yellow_F : Sign_Arrow_Large_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_Green_F : Sign_Arrow_Large_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_Pink_F : Sign_Arrow_Large_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Large_Blue_F : Sign_Arrow_Large_F {
    textPlural = "Помощник";
};
class Sign_Arrow_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Cyan_F : Sign_Arrow_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Yellow_F : Sign_Arrow_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Green_F : Sign_Arrow_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Direction_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Direction_Cyan_F : Sign_Arrow_Direction_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Direction_Green_F : Sign_Arrow_Direction_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Direction_Pink_F : Sign_Arrow_Direction_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Direction_Blue_F : Sign_Arrow_Direction_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Pink_F : Sign_Arrow_F {
    textPlural = "Помощник";
};
class Sign_Arrow_Blue_F : Sign_Arrow_F {
    textPlural = "Помощник";
};
class TestSphere_ACR : NonStrategic {
    textPlural = "Помощник";
};
class Sign_Sphere10cm_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Sphere100cm_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Sphere25cm_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Pointer_F : Helper_Base_F {
    textPlural = "Помощник";
};
class Sign_Pointer_Cyan_F : Sign_Pointer_F {
    textPlural = "Помощник";
};
class Sign_Pointer_Yellow_F : Sign_Pointer_F {
    textPlural = "Помощник";
};
class Sign_Pointer_Green_F : Sign_Pointer_F {
    textPlural = "Помощник";
};
class Sign_Pointer_Pink_F : Sign_Pointer_F {
    textPlural = "Помощник";
};
class Sign_Pointer_Blue_F : Sign_Pointer_F {
    textPlural = "Помощник";
};
class Fort_RazorWirePreview : Fort_RazorWire {
    textPlural = "Помощник";
};
class Cube5cm_ACR : Helper_Base_EP1 {
    textPlural = "Помощник";
};
class Cube25cm_ACR : Cube5cm_ACR {
    textPlural = "Помощник";
};
class TargetCenter : House_F {
    textPlural = "Помощник";
};
class TargetSector02 : TargetCenter {
    textPlural = "Помощник";
};
class TargetSector03 : TargetCenter {
    textPlural = "Помощник";
};
class TargetSector04 : TargetCenter {
    textPlural = "Помощник";
};
class TargetSector05 : TargetCenter {
    textPlural = "Помощник";
};
class WarfareBMGNest_Preview : StaticCannon_Preview {
    textPlural = "Помощник";
};
class ProtectionZone_F : House_F {
    textPlural = "Помощник";
};
class ProtectionZone_Invisible_F : ProtectionZone_F {
    textPlural = "Помощник";
};
class Training_target_EP1 : House {
    textPlural = "Помощник";
};














//----------------------------------------------
//				Прибрежная зона
//----------------------------------------------
class Land_Sunshade_01_F : Camping_base_F {
    textPlural = "Прибрежная зона";
};
class Land_Sunshade_02_F : Land_Sunshade_01_F {
    textPlural = "Прибрежная зона";
};
class Land_Sunshade_03_F : Land_Sunshade_01_F {
    textPlural = "Прибрежная зона";
};
class Land_Sunshade_04_F : Land_Sunshade_01_F {
    textPlural = "Прибрежная зона";
};
class Land_BeachBooth_01_F : NonStrategic {
    textPlural = "Прибрежная зона";
};
class Land_Sunshade_F : Camping_base_F {
    textPlural = "Прибрежная зона";
    displayName = "Тент от солнца (синий)";
};
class Land_LifeguardTower_01_F : House_Small_f {
    textPlural = "Прибрежная зона";
};
class Land_LightHouse_F : House_F {
    textPlural = "Прибрежная зона";
};
class Land_Lighthouse_small_F : House_Small_f {
    textPlural = "Прибрежная зона";
};
class Land_Pier_small_F : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_Pier_wall_F : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_nav_pier_m_F : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_Pier_F : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_Pier_addon : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_FishingGear_01_F : Garbage_base_F {
    textPlural = "Прибрежная зона";
};
class Land_FishingGear_02_F : Garbage_base_F {
    textPlural = "Прибрежная зона";
};
class Land_sunshade_EP1 : House_EP1 {
    textPlural = "Прибрежная зона";
    displayName = "";
};
class Land_majak : House {
    textPlural = "Прибрежная зона";
    displayName = "";
};
class Land_majak2 : Land_majak {
    textPlural = "Прибрежная зона";
    displayName = "";
};
class Land_majak_podesta : House {
    textPlural = "Прибрежная зона";
    displayName = "Основание для маяка";
};
class Land_molo_beton : House {
    textPlural = "Прибрежная зона";
    displayName = "Пирс из камня со спусками";
};
class Land_molo_drevo_bs : House {
    textPlural = "Прибрежная зона";
    displayName = "Пирс длинный (деревянный)";
};
class Land_molo_drevo_end : House {
    textPlural = "Прибрежная зона";
    displayName = "Пирс (спуск, деревянный)";
};

class Land_molo_krychle : House {
    textPlural = "Прибрежная зона";
    displayName = "Пирс из камня";
};
class Land_Nav_Boathouse : House {
    textPlural = "Прибрежная зона";
    displayName = "Ангар для двух катеров (деревянный, старый)";
};
class Land_Nav_Boathouse_Pier : Land_Nav_Boathouse {
    textPlural = "Прибрежная зона";
    displayName = "Жеревянные мостики";
};
class Land_Nav_Boathouse_PierL : House {
    textPlural = "Прибрежная зона";
    displayName = "Жеревянные мостик L";
};

class Land_Nav_Boathouse_PierR : Land_Nav_Boathouse_PierL {
    textPlural = "Прибрежная зона";
    displayName = "Жеревянные мостик R";
};
class Land_Nav_Boathouse_PierT : Land_Nav_Boathouse_PierL {
    textPlural = "Прибрежная зона";
    displayName = "Жеревянные мостик длинный";
};
class Land_NAV_Lighthouse : House {
    textPlural = "Прибрежная зона";
    displayName = "Маяк";
};
class Land_NAV_Lighthouse2 : Land_NAV_Lighthouse {
    textPlural = "Прибрежная зона";
    displayName = "Маяк";
};
class Land_nav_pier_m_2 : House {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс m2";
};
class land_nav_pier_c : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс 1";
};
class land_nav_pier_c2 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс 2";
};
class land_nav_pier_c2_end : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс 3";
};
class land_nav_pier_c_270 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс угол";
};
class land_nav_pier_c_90 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс 4";
};
class land_nav_pier_c_big : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс 5";
};
class land_nav_pier_C_L : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс L";
};
class land_nav_pier_C_L10 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс L10";
};
class land_nav_pier_C_L30 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс L30";
};
class land_nav_pier_C_R : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс R";
};
class land_nav_pier_C_R10 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс R10";
};
class land_nav_pier_C_R30 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс R30";
};
class land_nav_pier_c_t15 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс t15";
};
class land_nav_pier_c_t20 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс t20";
};
class land_nav_pier_F_17 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Пирс (опора 17)";
};
class land_nav_pier_F_23 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Пирс (опора 23)";
};
class land_nav_pier_m : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс m";
};
class land_nav_pier_m_1 : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс m1";
};

class land_nav_pier_m_end : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс m_end";
};
class land_nav_pier_M_fuel : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Бетонный пирс M_fuel";
};
class land_nav_pier_pneu : Land_nav_pier_m_2 {
    textPlural = "Прибрежная зона";
    displayName = "Пирс (шины)";
};
class Land_Pier_Box_F : Piers_base_F {
    textPlural = "Прибрежная зона";
};
class Land_CrabCages_F : Garbage_base_F {
    textPlural = "Прибрежная зона";
};

//----------------------------------------------
//				Обломки
//----------------------------------------------
class Scrapyard_base_F;
class Land_ScrapHeap_2_F : Scrapyard_base_F {
    textPlural = "Обломки";
};
class Land_Scrap_MRAP_01_F : Scrapyard_base_F {
    textPlural = "Обломки";
};
class Land_ScrapHeap_1_F : Scrapyard_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_BRDM2_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Car_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Offroad2_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Offroad_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Truck_dropside_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Traw2_F : UWreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Traw_F : UWreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Car2_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Car3_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Skodovka_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_UWreck_MV22_F : UWreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Heli_Attack_01_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Truck_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Hunter_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_BMP2_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_UWreck_Heli_Attack_02_F : UWreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Heli_Attack_02_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Slammer_turret_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Slammer_hull_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Slammer_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_T72_turret_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_T72_hull_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_UAZ_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Ural_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Van_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_Plane_Transport_01_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_HMMWV_F : Wreck_base_F {
    textPlural = "Обломки";
};
class Land_Wreck_CarDismantled_F : Wreck_base_F {
    textPlural = "Обломки";
};
class BMP2Wreck : Wreck_Base {
    textPlural = "Обломки";
};
class BRDMWreck : Wreck_Base {
    textPlural = "Обломки";
};
class datsun01Wreck : Wreck_Base {
    textPlural = "Обломки";
};
class datsun02Wreck : Wreck_Base {
    textPlural = "Обломки";
};
class hiluxWreck : Wreck_Base {
    textPlural = "Обломки";
};
class LADAWreck : Wreck_Base {
    textPlural = "Обломки";
};
class Mi8Wreck : Wreck_Base {
    textPlural = "Обломки";
};
class SKODAWreck : Wreck_Base {
    textPlural = "Обломки";
};
class T72Wreck : Wreck_Base {
    textPlural = "Обломки";
};
class T72WreckTurret : Wreck_Base {
    textPlural = "Обломки";
};
class UAZWreck : Wreck_Base {
    textPlural = "Обломки";
};
class UralWreck : Wreck_Base {
    textPlural = "Обломки";
};




//----------------------------------------------
//				Строения
//----------------------------------------------
class Land_GH_Pool_F : NonStrategic {
    textPlural = "Строения";
};
class Land_GH_Stairs_F : NonStrategic {
    textPlural = "Строения";
};
class Land_GH_Platform_F : NonStrategic {
    textPlural = "Строения";
};







//----------------------------------------------
//				Военные (антураж)
//----------------------------------------------
class Land_Pallet_MilBoxes_F : Scrapyard_base_F {
    textPlural = "Военные (антураж)";
};
class Land_Device_assembled_F : Strategic {
    textPlural = "Военные (антураж)";
};
class Land_Device_disassembled_F : Land_Device_assembled_F {
    textPlural = "Военные (антураж)";
};
class Land_PaperBox_closed_F : Scrapyard_base_F {
    textPlural = "Военные (антураж)";
};
class Land_PaperBox_open_full_F : Scrapyard_base_F {
    textPlural = "Военные (антураж)";
};
class Land_PaperBox_open_empty_F : Scrapyard_base_F {
    textPlural = "Военные (антураж)";
};
class 76n6ClamShell : Strategic {
    textPlural = "Военные (антураж)";
    displayName = "Радар (зеленый)";
};
class 76n6ClamShell_EP1 : 76n6ClamShell {
    textPlural = "Военные (антураж)";
    displayName = "Радар (Песчанный)";
};
class Explosive : Small_items_NoInteractive {
    textPlural = "Военные (антураж)";
    displayName = "С4";
};
class Gunrack1 : NonStrategic {
    textPlural = "Военные (антураж)";
    displayName = "Подставка под оружие";
};
class Hedgehog : Strategic {
    textPlural = "Военные (антураж)";
    displayName = "противотанковй еж (старый)";
};
class Land_podlejzacka : Strategic {
    textPlural = "Военные (антураж)";
    displayName = "Препядствие (ржавое)";
};
class Land_obstacle_get_over : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Ящики (металл и пленка)";
};
class Land_obstacle_prone : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Ящик (пленка, длинный)";
};
class Land_obstacle_run_duck : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Ящики (металл 2)";
};
class Land_obihacka : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Бег (ржавое)";
};
class Land_prebehlavka : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Мост (дерево)";
};
class Land_prolejzacka : Land_podlejzacka {
    textPlural = "Военные (антураж)";
    displayName = "Препядствие - окно(дерево)";
};
class ZavoraAnim : NonStrategic {
    textPlural = "Военные (антураж)";
    displayName = "Щлагбаун";
};
class Land_MapBoard_F : thingX {
    textPlural = "Военные (антураж)";
};
class MapBoard_stratis_F : Land_MapBoard_F {
    textPlural = "Военные (антураж)";
};
class Box_IND_AmmoOrd_F : IND_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_East_AmmoOrd_F : EAST_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_IND_Grenades_F : IND_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_East_Grenades_F : EAST_Box_Base {
    textPlural = "Военные (антураж)";
};
class MapBoard_altis_F : Land_MapBoard_F {
    textPlural = "Военные (антураж)";
};
class MapBoard_seismic_F : Land_MapBoard_F {
    textPlural = "Военные (антураж)";
};
class Box_IND_Wps_F : IND_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_East_Wps_F : EAST_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_IND_Ammo_F : IND_Box_Base {
    textPlural = "Военные (антураж)";
};
class Box_East_Ammo_F : EAST_Box_Base {
    textPlural = "Военные (антураж)";
};
class PowGen_Big : House {
    textPlural = "Военные (антураж)";
    displayName = "Генератор (лес)";
};
class PowGen_Big_EP1 : PowGen_Big {
    textPlural = "Военные (антураж)";
    displayName = "Генератор (песок)";
};








//----------------------------------------------
//				Военные (антураж)
//----------------------------------------------

class CDF_WarfareBFieldhHospital : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская";
};
class GUE_WarfareBFieldhHospital : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская";
};
class INS_WarfareBFieldhHospital : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская";
};
class MASH : Camp_base {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская + генератор";
};
class MASH_EP1 : MASH {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская + генератор";
};
class RU_WarfareBFieldhHospital : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Палатка медицинская";
};
class TK_GUE_WarfareBFieldhHospital_Base_EP1 : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Штаб (песок)";
};
class TK_GUE_WarfareBFieldhHospital_EP1 : TK_GUE_WarfareBFieldhHospital_Base_EP1 {
    textPlural = "Военные (палатки)";
    displayName = "Штаб (песок)";
};
class TK_WarfareBFieldhHospital_Base_EP1 : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Штаб (песок)";
};
class TK_WarfareBFieldhHospital_EP1 : TK_WarfareBFieldhHospital_Base_EP1 {
    textPlural = "Военные (палатки)";
    displayName = "Штаб (песок)";
};
class US_WarfareBFieldhHospital_Base_EP1 : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Штаб 2 (песок)";
};
class US_WarfareBFieldhHospital_EP1 : US_WarfareBFieldhHospital_Base_EP1 {
    textPlural = "Военные (палатки)";
    displayName = "Штаб 2 (песок)";
};
class USMC_WarfareBFieldhHospital : BASE_WarfareBFieldhHospital {
    textPlural = "Военные (палатки)";
    displayName = "Штаб 2 (лес)";
};




//----------------------------------------------
//				Тренировка
//----------------------------------------------
class Land_Obstacle_Bridge_F : NonStrategic {
    textPlural = "Тренировка";
};
class Land_Obstacle_RunAround_F : Land_Obstacle_Bridge_F {
    textPlural = "Тренировка";
};
class Land_Obstacle_Cross_F : Land_Obstacle_Bridge_F {
    textPlural = "Тренировка";
};
class Land_Obstacle_Saddle_F : NonStrategic {
    textPlural = "Тренировка";
};
class Land_Obstacle_Pass_F : Land_Obstacle_Bridge_F {
    textPlural = "Тренировка";
};
class Land_Obstacle_Crawl_F : Land_Obstacle_Bridge_F {
    textPlural = "Тренировка";
};
class Land_Obstacle_Ramp_F : Land_Obstacle_Bridge_F {
    textPlural = "Тренировка";
};
class Obstacle_saddle : NonStrategic {
    textPlural = "Тренировка";
};


//----------------------------------------------
//				АЗС
//----------------------------------------------
class Land_A_FuelStation_Feed : Strategic {
    textPlural = "АЗС";
    displayName = "Станция (старая)";
};
class Land_FuelStation_Feed_PMC : Strategic {
    textPlural = "АЗС";
    displayName = "Станция";
};
class Land_Ind_FuelStation_Feed_EP1 : Strategic {
    textPlural = "АЗС";
    displayName = "Станция (новая)";
};
class Land_fuel_tank_small : Strategic {
    textPlural = "АЗС";
    displayName = "Газовое хранилище (короткое)";
};
class Land_Fuel_tank_stairs : Land_fuel_tank_small {
    textPlural = "АЗС";
    displayName = "Газовое хранилище";
};
class Land_Fuel_tank_stairs_ep1 : Strategic {
    textPlural = "АЗС";
    displayName = "Газовое хранилище (светлее)";
};
class Land_CarService_F : House_F {
    textPlural = "АЗС";
};
class Land_FuelStation_Sign_F : House_Small_F {
    textPlural = "АЗС";
};
class Land_FuelStation_Feed_F : House_Small_F {
    textPlural = "АЗС";
};
class Land_FuelStation_Build_F : House_Small_F {
    textPlural = "АЗС";
};
class Land_FuelStation_Shed_F : House_F {
    textPlural = "АЗС";
};
class Land_A_FuelStation_Build : House {
    textPlural = "АЗС";
    displayName = "ПОШОЛ касса";
};
class Land_A_FuelStation_Shed : House {
    textPlural = "АЗС";
    displayName = "ПОШОЛ навес";
};
class Land_FuelStation_Build_PMC : House_EP1 {
    textPlural = "АЗС";
    displayName = "касса (руины)";
};
class Land_FuelStation_Shed_PMC : House_EP1 {
    textPlural = "АЗС";
    displayName = "Навес (руины)";
};
class Land_fs_price_F : House_Small_f {
    textPlural = "АЗС";
};
class Land_fs_feed_F : House_Small_f {
    textPlural = "АЗС";
};
class Land_fs_roof_F : House_F {
    textPlural = "АЗС";
};
class Land_fs_sign_F : House_Small_f {
    textPlural = "АЗС";
};
class Land_Ind_FuelStation_Build_EP1 : House_EP1 {
    textPlural = "АЗС";
    displayName = "Shnell касса";
};
class Land_Ind_FuelStation_Shed_EP1 : House_EP1 {
    textPlural = "АЗС";
    displayName = "Shnell навес";
};







//----------------------------------------------
//				Контейнер
//----------------------------------------------
class Base_WarfareBContructionSite : Strategic {
    textPlural = "Контейнер";
    displayName = "Контейнер (зеленый, старый)";
};
class CDF_WarfareBContructionSite : Base_WarfareBContructionSite {
    textPlural = "Контейнер";
    displayName = "Контейнер (красный, старый)";
};
class CDF_WarfareBContructionSite1 : CDF_WarfareBContructionSite {
    textPlural = "Контейнер";
    displayName = "Контейнер (Песчайнный, старый)";
};
class Fort_Crate_wood : NonStrategic {
    textPlural = "Контейнер";
    displayName = "Ящик (деревянный)";
};
class Gue_WarfareBContructionSite : Base_WarfareBContructionSite {
    textPlural = "Контейнер";
    displayName = "Контейнер (зеленый, старый)";
};
class Gue_WarfareBContructionSite1 : Gue_WarfareBContructionSite {
    textPlural = "Контейнер";
    displayName = "Контейнер AHKER(Песчайнный)";
};
class Misc_cargo_cont_net1 : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Ящик x1";
};
class Misc_cargo_cont_net2 : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Ящик x3";
};
class Misc_cargo_cont_net3 : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Ящик x7";
};
class Misc_cargo_cont_small : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Контейнер (зеленый, куб)";
};
class Misc_cargo_cont_small2 : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Контейнер (белый, куб)";
};
class Misc_cargo_cont_small_EP1 : Misc_cargo_cont_small {
    textPlural = "Контейнер";
    displayName = "Контейнер (зеленый, куб)";
};
class Misc_cargo_cont_tiny : Military_Item_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Контейнер (металл, куб)";
};
class Misc_palletsfoiled : Misc_thing_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Контейнер (пленка, куб)";
};
class Misc_palletsfoiled_heap : Misc_thing_NoInteractive {
    textPlural = "Контейнер";
    displayName = "Контейнер x7(пленка, куб)";
};
class Land_CinderBlocks_F : Constructions_base_F {
    textPlural = "Контейнер";
};
class Box_NATO_AmmoVeh_F : NATO_Box_Base {
    textPlural = "Контейнер";
};
class Box_IND_AmmoVeh_F : IND_Box_Base {
    textPlural = "Контейнер";
};
class Box_East_AmmoVeh_F : EAST_Box_Base {
    textPlural = "Контейнер";
};
class Land_Cargo40_white_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_cyan_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_light_blue_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_yellow_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_military_green_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_brick_red_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_red_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_orange_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_sand_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_light_green_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_grey_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo40_blue_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_white_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_cyan_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_light_blue_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_yellow_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_military_green_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_brick_red_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_red_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_orange_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_sand_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_light_green_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_grey_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Cargo20_blue_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_CargoBox_V1_F : thingX {
    textPlural = "Контейнер";
};
class Land_MetalBarrel_F : Items_base_F {
    textPlural = "Контейнер";
};
class Unwrapped_sleeping_bag : Misc_thing {
    textPlural = "Контейнер";
};
class Wheel_barrow : Misc_thing {
    textPlural = "Контейнер";
};
class Wooden_barrel : BarrelBase {
    textPlural = "Контейнер";
};
class Wooden_barrels : Wooden_barrel {
    textPlural = "Контейнер";
};
class Land_Cargo20_vr_F : Cargo_base_F {
    textPlural = "Контейнер";
};
class Land_Misc_Cargo1B : House {
    textPlural = "Контейнер";
    displayName = "";
};
class Land_Misc_Cargo1Ao_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (отк, синий)";
};
class Land_Misc_Cargo1Bo : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (отк, красный)";
};
class Land_Misc_Cargo1Bo_EP1 : Land_Misc_Cargo1Ao_EP1 {
    textPlural = "Контейнер";
    displayName = "Контейнер (отк, черный)";
};
class Misc_Cargo1Bo_military : Land_Misc_Cargo1Bo {
    textPlural = "Контейнер";
    displayName = "Контенер зеленый (отк)";
};
class Misc_Cargo1B_military : Land_Misc_Cargo1B {
    textPlural = "Контейнер";
    displayName = "Контенер зеленый";
};
class Land_Misc_Cargo1E_EP1 : House {
    textPlural = "Контейнер";
    displayName = "";
};
class Land_Misc_Cargo1Eo_EP1 : Land_Misc_Cargo1Bo_EP1 {
    textPlural = "Контейнер";
    displayName = "";
};
class Land_Misc_Cargo2E : House {
    textPlural = "Контейнер";
    displayName = "";
};
class Land_Misc_Cargo2E_EP1 : House {
    textPlural = "Контейнер";
    displayName = "";
};
class Land_Misc_Cargo1A_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (отк, красный)";
};

class Land_Misc_Cargo1B_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (черный)";
};

class Land_Misc_Cargo1C : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (песок)";
};
class Land_Misc_Cargo1C_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (красный)";
};
class Land_Misc_Cargo1D : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (красный)";
};
class Land_Misc_Cargo1D_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (красный)";
};
class Land_Misc_Cargo1E : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (голубой)"
};
class Land_Misc_Cargo1F : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (синий)"
};
class Land_Misc_Cargo1G : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (песок)";
};
class Land_Misc_Cargo2A_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, черныe)"
};
class Land_Misc_Cargo2B : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, песок)"
};
class Land_Misc_Cargo2B_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, черныe)"
};
class Land_Misc_Cargo2C : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, красный)"
};
class Land_Misc_Cargo2C_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, синий)"
};
class Land_Misc_Cargo2D : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, красный)"
};
class Land_Misc_Cargo2D_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (x2, зеленый)"
};
class Land_Misc_CargoMarket1a_EP1 : House {
    textPlural = "Контейнер";
    displayName = "Контейнер (отк, черный)";
};






//----------------------------------------------
//				Могилы
//----------------------------------------------
class Grave : NonStrategic {
    textPlural = "Могилы";
    displayName = "Могила (землянная)";
};
class Body : Grave {
    textPlural = "Могилы";
    displayName = "Тело под пленкой";
};
class GraveCross1 : Grave {
    textPlural = "Могилы";
    displayName = "Крест (деревянный с жетоном)";
};
class GraveCross2 : Grave {
    textPlural = "Могилы";
    displayName = "Крест (деревянный)";
};
class GraveCrossHelmet : Grave {
    textPlural = "Могилы";
    displayName = "Крест (деревянный с каской)";
};
class Hanged : NonStrategic {
    textPlural = "Могилы";
    displayName = "Повешанный";
};
class Hanged_MD : Hanged {
    textPlural = "Могилы";
    displayName = "Повешанный (доктор)";
};
class Land_HumanSkeleton_F : Dead_base_F {
    textPlural = "Могилы";
};
class Land_Grave_dirt_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_rocks_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_forest_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_memorial_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_obelisk_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_monument_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_soldier_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_V1_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_V2_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Grave_V3_F : Cemetery_base_F {
    textPlural = "Могилы";
};
class Land_Calvary_02_V2_F : House_Small_f {
    textPlural = "Могилы";
};
class Land_Calvary_01_V1_F : House_Small_f {
    textPlural = "Могилы";
};



//----------------------------------------------
//				Такистан (антураж)
//----------------------------------------------
class Land_Blankets_EP1 : Thing_NoInteractive_EP1 {
    textPlural = "Такистан (антураж)";
    displayName = "Ковры 3 (свернутые)";
};
class Land_Boots_EP1 : Thing_NoInteractive_EP1 {
    textPlural = "Такистан (антураж)";
    displayName = "Обувь";
};
class Land_Carpet_2_EP1 : Thing_NoInteractive_EP1 {
    textPlural = "Такистан (антураж)";
    displayName = "Ковер (полусвернутый)";
};
class Land_Carpet_EP1 : Thing_NoInteractive_EP1 {
    textPlural = "Такистан (антураж)";
    displayName = "Кровать (низкая)";
};


//----------------------------------------------
//				Замок
//----------------------------------------------
class Land_AncientPillar_damaged_F : thingX {
    textPlural = "Замок";
};
class Land_AncientPillar_fallen_F : thingX {
    textPlural = "Замок";
};
class Land_AncientPillar_F : thingX {
    textPlural = "Замок";
};
class Land_Amphitheater_F : House_F {
    textPlural = "Замок";
};
class Land_A_Castle_Bastion : House {
    textPlural = "Замок";
};
class Land_A_Castle_Bergfrit : House {
    textPlural = "Замок";
};
class Land_A_Castle_Bergfrit_dam : Land_A_Castle_Bergfrit {
    textPlural = "Замок";
};
class Land_A_Castle_Donjon : House {
    textPlural = "Замок";
};
class Land_A_Castle_Donjon_dam : Land_A_Castle_Donjon {
    textPlural = "Замок";
};
class Land_A_Castle_Gate : House {
    textPlural = "Замок";
};
class Land_A_Castle_Stairs_A : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_20 : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_20_Turn : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_Corner : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_Corner_2 : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_End : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall1_End_2 : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall2_30 : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall2_Corner : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall2_Corner_2 : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall2_End : House {
    textPlural = "Замок";
};
class Land_A_Castle_Wall2_End_2 : House {
    textPlural = "Замок";
};
class Land_A_Castle_WallS_10 : House {
    textPlural = "Замок";
};
class Land_A_Castle_WallS_5_D : House {
    textPlural = "Замок";
};
class Land_A_Castle_WallS_End : House {
    textPlural = "Замок";
};
class Land_Castle_01_wall_01_F : House_F {
    textPlural = "Замок";
};
class Land_Castle_01_house_ruin_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_step_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_tower_F : House_F {
    textPlural = "Замок";
};
class Land_Castle_01_church_a_ruin_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_church_b_ruin_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_church_ruin_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_02_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_03_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_04_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_05_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_06_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_07_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_08_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_09_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_10_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_11_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_12_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_13_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_14_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_15_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};
class Land_Castle_01_wall_16_F : Land_Castle_01_wall_01_F {
    textPlural = "Замок";
};


//----------------------------------------------
//				Здания (Стройка)
//----------------------------------------------
class Land_A_BuildingWIP : House {
    textPlural = "Здания (Стройка)";
    displayName = "Стройка";
};
class Land_A_BuildingWIP_EP1 : House_EP1 {
    textPlural = "Здания (Стройка)";
    displayName = "Стройка (песок)";
};
class Land_A_Crane_02a : House {
    textPlural = "Здания (Стройка)";
    displayName = "Кран пирс (основание)";
};
class Land_A_Crane_02b : House {
    textPlural = "Здания (Стройка)";
    displayName = "Кран пирс (башня)";
};
class Land_A_CraneCon : House {
    textPlural = "Здания (Стройка)";
    displayName = "Кран (желтый)";
};
class Land_Crane_F : House_F {
    textPlural = "Здания (Стройка)";
};
class Land_Misc_IronPipes_EP1 : House_EP1 {
    textPlural = "Здания (Стройка)";
};
class Land_Misc_ConcPipeline_EP1 : House_EP1 {
    textPlural = "Здания (Стройка)";
};
class Land_Misc_ConcOutlet_EP1 : House_EP1 {
    textPlural = "Здания (Стройка)";
};
class Land_Misc_Coil_EP1 : House_EP1 {
    textPlural = "Здания (Стройка)";
};
class Land_leseni2x : House {
    textPlural = "Здания (Стройка)";
    displayName = "Леса x2";
};
class Land_leseni4x : House {
    textPlural = "Здания (Стройка)";
    displayName = "Леса x4";
};
class Land_WIP_F : House_F {
    textPlural = "Здания (Стройка)";
};
class Land_Unfinished_Building_02_F : House_F {
    textPlural = "Здания (Стройка)";
};
class Land_Unfinished_Building_01_F : House_F {
    textPlural = "Здания (Стройка)";
};


//----------------------------------------------
//				Здания (Черно)
//----------------------------------------------
class Land_A_MunicipalOffice : NonStrategic {
    textPlural = "Здания (Черно)";
    displayName = "Офис с часами";
};
class Land_ReservoirTower_F : House_F {
    textPlural = "Здания (Черно)";
};
class Land_A_GeneralStore_01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Магазин Звезда (лестнциа)";
};
class Land_A_GeneralStore_01_dam : Land_A_GeneralStore_01 {
    textPlural = "Здания (Черно)";
    displayName = "Магазин Звезда (лестнциа, руины)";
};
class Land_A_GeneralStore_01a : Land_A_GeneralStore_01 {
    textPlural = "Здания (Черно)";
    displayName = "Магазин Звезда";
};
class Land_A_GeneralStore_01a_dam : Land_A_GeneralStore_01_dam {
    textPlural = "Здания (Черно)";
    displayName = "Магазин Звезда (руины)";
};
class Land_A_Hospital : House {
    textPlural = "Здания (Черно)";
    displayName = "Больница";
};
class Land_A_Hospital_dam : Land_A_Hospital {
    textPlural = "Здания (Черно)";
    displayName = "Больница (руины)";
};
class Land_A_Office01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Офис (4 этажа)";
};
class Land_A_Office01_EP1 : House_EP1 {
    textPlural = "Здания (Черно)";
    displayName = "Офис 2 (4 этажа)";
};
class Land_A_Office02 : House {
    textPlural = "Здания (Черно)";
    displayName = "Офис 3 (4 этажа)";
};
class Land_A_Office02_dam : Land_A_Office02 {
    textPlural = "Здания (Черно)";
    displayName = "Офис 3 (4 этажа, руины)";
};
class Land_A_Pub_01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом (угол, арка)";
};
class Land_a_stationhouse : House {
    textPlural = "Здания (Черно)";
    displayName = "Пожарная станция (красная)";
};
class Land_A_Stationhouse_ep1 : House_EP1 {
    textPlural = "Здания (Черно)";
    displayName = "Пожарная станция (синяя)";
};
class Land_A_statue01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Статуя (солдаты)";
};
class Land_A_statue02 : House {
    textPlural = "Здания (Черно)";
    displayName = "Статуя (танк)";
};
class Land_A_Statue_EP1 : House_EP1 {
    textPlural = "Здания (Черно)";
    displayName = "Статуя (солдаты, ржавая)";
};
class Land_A_TVTower_Base : House {
    textPlural = "Здания (Черно)";
    displayName = "ТВ (основание)";
};
class Land_A_TVTower_Mid : House {
    textPlural = "Здания (Черно)";
    displayName = "ТВ (середины)";
};
class Land_A_TVTower_Top : House {
    textPlural = "Здания (Черно)";
    displayName = "ТВ (верхушка)";
};
class Land_afbarabizna : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом";
};
class Land_afdum_mesto2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом 2";
};
class Land_afdum_mesto2L : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом (угол)";
};
class Land_afdum_mesto3 : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом (арка)";
};
class Land_afhospoda_mesto : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом 2 (угол)";
};
class Land_dulni_bs : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревший дом 3";
};

class Land_Barn_Metal : House {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (большой, красный)";
};
class Land_Barn_Metal_dam : Land_Barn_Metal {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (большой, красный, разрушенный)";
};
class Land_Barn_W_01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Ангар";
};
class Land_Barn_W_01_dam : Land_Barn_W_01 {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (разрушенный)";
};
class Land_Barn_W_02 : Land_Barn_W_01 {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (маленький)";
};
class Land_bouda1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай (2-х этажный)";
};
class Land_bouda2_vnitrek : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай";
};
class Land_bouda3 : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай (большой)";
};
class Land_bouda_plech : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай (миниы)";
};
class Land_brana02nodoor : House {
    textPlural = "Здания (Черно)";
    displayName = "Арка";
};
class Land_budova1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом_1";
};
class Land_budova2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом_2";
};
class Land_budova4 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом_4";
};
class Land_budova5 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом_5";
};
class Land_Church_01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (желтая)";
};
class Land_Church_02 : House {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (большая, серая)";
};
class Land_Church_02a : Land_Church_02 {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (большая, серая, купол)";
};
class Land_Church_03 : House {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (большая, белая)";
};
class Land_Church_03_dam : Land_Church_03 {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (большая, белая, разрушенная)";
};
class Land_Church_05R : House {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (руины)";
};
class Land_kostel_trosky : House {
    textPlural = "Здания (Черно)";
    displayName = "Церковь (руины 2)";
};
class Land_cihlovej_dum : House {
    textPlural = "Здания (Черно)";
    displayName = "Домик (камень, красный)";
};
class Land_cihlovej_dum_in : House {
    textPlural = "Здания (Черно)";
    displayName = "Домик2 (камень, красный)";
};
class Land_Company3_2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Вывеска организации";
};
class Land_Dam_Conc_20 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дамба 2";
};
class Land_Dam_Barrier_40 : Land_Dam_Conc_20 {
    textPlural = "Здания (Черно)";
    displayName = "Дамба";
};
class Land_Dam_ConcP_20 : Land_Dam_Conc_20 {
    textPlural = "Здания (Черно)";
    displayName = "Дамба 3";
};
class Land_deutshe : House {
    textPlural = "Здания (Черно)";
    displayName = "Здание (2-х этажное)";
};
class Land_deutshe_mini : House {
    textPlural = "Здания (Черно)";
    displayName = "Здание (1-х этажное)";
};
class Land_domek_rosa : House {
    textPlural = "Здания (Черно)";
    displayName = "Домик (красное)";
};
class Land_dum01 : House {
    textPlural = "Здания (Черно)";
    displayName = "Бараки";
};
class Land_dum_m2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Домик";
};
class Land_dum_mesto : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом 5";
};
class Land_dum_mesto2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом 6";
};
class Land_dum_mesto2l : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом 7";
};
class Land_dum_mesto3 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом 8";
};
class Land_dum_mesto_in : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом 9";
};
class Land_dum_zboreny : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревшее здание";
};
class Land_dum_zboreny_total : House {
    textPlural = "Здания (Черно)";
    displayName = "Сгоревшее здание (руины)";
};
class Land_Farm_Cowshed_a : House {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 1";
};
class Land_Farm_Cowshed_b : House {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 2";
};
class Land_Farm_Cowshed_c : House {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 3";
};
class Land_Farm_Cowshed_a_dam : Land_Farm_Cowshed_a {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 1 (руины)";
};
class Land_Farm_Cowshed_b_dam : Land_Farm_Cowshed_b {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 2 (руины)";
};
class Land_Farm_Cowshed_c_dam : Land_Farm_Cowshed_c {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник 3 (руины)";
};
class Land_Farm_WTower : House {
    textPlural = "Здания (Черно)";
    displayName = "Геостанция";
};
class Land_garaz_mala : House {
    textPlural = "Здания (Черно)";
    displayName = "Гараж";
};
class Land_garaz : House {
    textPlural = "Здания (Черно)";
    displayName = "Гараж (x3)";
};
class Land_Gate_IndVar2_5 : Land_Wall_Gate_Ind1_R {
    textPlural = "Здания (Черно)";
    displayName = "Ворота (деревянный)";
};
class Land_Gate_Wood1_5 : Land_Wall_Gate_Ind1_R {
    textPlural = "Здания (Черно)";
    displayName = "Калитка (деревянный)";
};
class Land_Gate_wood2_5 : Land_Wall_Gate_Ind1_R {
    textPlural = "Здания (Черно)";
    displayName = "Калитка 2 (деревянный)";
};
class Land_GeneralStore_01a_PMC : House {
    textPlural = "Здания (Черно)";
    displayName = "Магазин";
};
class Land_GeneralStore_01a_dam_PMC : Land_GeneralStore_01a_PMC {
    textPlural = "Здания (Черно)";
    displayName = "Магазин (руины)";
};
class Land_hospital : House {
    textPlural = "Здания (Черно)";
    displayName = "Больница (П)";
};

class Land_HouseB_Tenement : House {
    textPlural = "Здания (Черно)";
    displayName = "Гостиница";
};

class Land_HouseBlock_A1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 1";
};
class Land_HouseBlock_A1_1 : Land_HouseBlock_A1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 1_1";
};
class Land_HouseBlock_A1_2 : Land_HouseBlock_A1_1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 1_2";
};
class Land_HouseBlock_A2 : Land_HouseBlock_A1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 2";
};
class Land_HouseBlock_A2_1 : Land_HouseBlock_A1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 2_1";
};
class Land_HouseBlock_A3 : Land_HouseBlock_A1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок А 3";
};
class Land_HouseBlock_B1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 1";
};
class Land_HouseBlock_B2 : Land_HouseBlock_B1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 2";
};
class Land_HouseBlock_B3 : Land_HouseBlock_B1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 3";
};
class Land_HouseBlock_B4 : Land_HouseBlock_B1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 4";
};
class Land_HouseBlock_B5 : Land_HouseBlock_B1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 5";
};
class Land_HouseBlock_B6 : Land_HouseBlock_B1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок Б 6";
};

class Land_HouseBlock_C1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Блок С 1";
};
class Land_HouseBlock_C2 : Land_HouseBlock_C1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок С 2";
};
class Land_HouseBlock_C3 : Land_HouseBlock_C1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок С 4";
};
class Land_HouseBlock_C4 : Land_HouseBlock_C1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок С 4";
};
class Land_HouseBlock_C5 : Land_HouseBlock_C1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок С 5";
};

class Land_HouseBlock_D1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Блок О 2";
};
class Land_HouseBlock_D2 : Land_HouseBlock_D1 {
    textPlural = "Здания (Черно)";
    displayName = "Блок О 2";
};
class Land_Tec : House {
    textPlural = "Здания (Черно)";
    displayName = "Вывеска TEC";
};
class Land_Panelak : House {
    textPlural = "Здания (Черно)";
    displayName = "Панедьный дом 2";
};
class Land_Panelak2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Панедьный дом 4";
};
class Land_Panelak3 : House {
    textPlural = "Здания (Черно)";
    displayName = "Панедьный дом 8";
};
class Pile_of_wood : Land_Ind_Timbers {
    textPlural = "Здания (Черно)";
    displayName = "Дрова";
};
class Land_psi_bouda : House {
    textPlural = "Здания (Черно)";
    displayName = "Собачья будка";
};
class Land_HouseV2_01A : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 1-a";
};
class Land_HouseV2_01A_dam : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 1-a (урон)";
};
class Land_HouseV2_01B : Land_HouseV2_01A {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 1-б";
};
class Land_HouseV2_01B_dam : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 1-б (урон)";
};
class Land_HouseV2_02 : Land_HouseV2_01A {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2";
};
class Land_HouseV2_02_Interier : Land_HouseV2_02 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 (вход, урон)";
};
class Land_HouseV2_02_Interier_dam : Land_HouseV2_02 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 2 (вход, урон)";
};
class Land_HouseV2_03 : Land_HouseV2_01A {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 3";
};
class Land_HouseV2_03_dam : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 3 (урон)";
};
class Land_HouseV2_03B : Land_HouseV2_01A {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 3б";
};
class Land_HouseV2_03B_dam : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 3б  (урон)";
};
class Land_HouseV2_04 : Land_HouseV2_01A {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 4";
};
class Land_HouseV2_04_interier : Land_HouseV2_04 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 4 (вход)";
};
class Land_HouseV2_04_interier_dam : Land_HouseV2_02_Interier_dam {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 4 (вход, урон)";
};
class Land_HouseV2_05 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V2 5";
};

class Land_HouseV_1I1 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1I1";
};
class Land_HouseV_1I2 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1I2";
};
class Land_HouseV_1I3 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1I3";
};
class Land_HouseV_1I4 : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1I4";
};
class Land_HouseV_1L1 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1L1";
};
class Land_HouseV_1L2 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1L2";
};
class Land_HouseV_1T : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 1T";
};
class Land_HouseV_2I : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 2I";
};
class Land_HouseV_2L : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 2L";
};
class Land_HouseV_2L_dam : House {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 2L (урон)";
};
class Land_houseV_2T1 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 2T1";
};
class Land_houseV_2T2 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 2T2";
};
class Land_HouseV_3I1 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 3T1";
};
class Land_HouseV_3I2 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 3T2";
};
class Land_HouseV_3I3 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 3T3";
};
class Land_HouseV_3I4 : Land_HouseV_1I2 {
    textPlural = "Здания (Черно)";
    displayName = "Дом V 3T4";
};
class Land_KBud : House {
    textPlural = "Здания (Черно)";
    displayName = "Туалет";
};
class Land_hut06 : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай";
};
class Land_hut_old02 : House {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (старый)";
};
class Land_Ind_PowerStation_EP1 : House_EP1 {
    textPlural = "Здания (Черно)";
    displayName = "Подстанция (красная)";
};
class Land_kulna : House {
    textPlural = "Здания (Черно)";
    displayName = "Сарай";
};
class Land_Misc_PowerStation : House {
    textPlural = "Здания (Черно)";
    displayName = "Подстанция";
};
class Land_Misc_WaterStation : House {
    textPlural = "Здания (Черно)";
    displayName = "Техбудка";
};
class Land_pumpa : House {
    textPlural = "Здания (Черно)";
    displayName = "Водяная колонка";
};
class Land_R_A_GeneralStore_01a : House {
    textPlural = "Здания (Черно)";
    displayName = "Магазин звезда (руины)";
};
class Land_R_Barn_Metal : Land_R_A_GeneralStore_01a {
    textPlural = "Здания (Черно)";
    displayName = "Ангар красный (большой, без обшивки)";
};
class Land_R_Barn_W_01 : Land_R_A_GeneralStore_01a {
    textPlural = "Здания (Черно)";
    displayName = "Ангар (руины)";
};
class Land_R_Farm_Cowshed_a : Land_R_A_GeneralStore_01a {
    textPlural = "Здания (Черно)";
    displayName = "Свинарник руины";
};






//----------------------------------------------
//				Здания (Завод)
//----------------------------------------------
class Land_dp_bigTank_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_dp_smallTank_F : House_Small_f {
    textPlural = "Здания (Завод)";
};
class Land_cmp_Tower_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_cmp_Hopper_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_dp_mainFactory_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_D_Mlyn_Vys : House {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (наклонный)";
};
class Land_D_Pec_Vez1 : House {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (высокий)";
};
class Land_D_Pec_Vez2 : House {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер2 (высокий)";
};
class Land_D_Vez_Mlyn : House {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер2 (наклонный)";
};
class Land_D_VSilo_Pec : House {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер3 (наклонный)";
};
class Land_hopper_old_PMC : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Хранилище (ржавое)";
};
class Land_Factory_Conv1_End_F : House_Small_f {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Conv1_10_F : House_Small_f {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Conv2_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Conv1_Main_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Tank_rust_F : House_Small_f {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Main_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Tunnel_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Factory_Hopper_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Vez_Silo : House {
    textPlural = "Здания (Завод)";
};
class Land_Vez_Pec : House {
    textPlural = "Здания (Завод)";
};
class Land_IronPipes_F : NonStrategic {
    textPlural = "Здания (Завод)";
};
class Land_ConcretePipe_F : Constructions_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_valve_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_90degL_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_90degR_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_ground_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_20m_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe1_Uup_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_ground1_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_ground2_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_support_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_bigL_L_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_bigL_R_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_18_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_18ladder_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_big_9_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_Small_ground1_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_Small_ground2_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_SmallL_L_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_SmallL_R_F : IndPipe_Small_base_F {
    textPlural = "Здания (Завод)";
};
class Land_IndPipe2_Small_9_F : IndPipe_base_F {
    textPlural = "Здания (Завод)";
};
class Land_u_Shed_Ind_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_i_Shed_Ind_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_ReservoirTank_V1_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_ReservoirTank_Rust_F : House_F {
    textPlural = "Здания (Завод)";
};
class Land_Ind_Coltan_Conv1_10_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер";
};
class Land_Ind_Coltan_Conv1_end_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (край)";
};
class Land_Ind_Coltan_Conv1_Main_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (осн)";
};
class Land_Ind_Coltan_Conv2_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (наклон)";
};
class Land_Ind_Coltan_Hopper_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (баки)";
};
class Land_Ind_Coltan_Main_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Конвеер (здание)";
};
class Land_Ind_Expedice_1 : House {
    textPlural = "Здания (Завод)";
    displayName = "Завод";
};
class Land_Ind_Expedice_2 : House {
    textPlural = "Здания (Завод)";
    displayName = "Трубы";
};
class Land_Ind_Expedice_3 : House {
    textPlural = "Здания (Завод)";
    displayName = "Трубы (лестницы)";
};
class Land_Ind_Garage01 : House {
    textPlural = "Здания (Завод)";
    displayName = "Гараж (откр)";
};
class Land_Ind_Garage01_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Гараж";
};
class Land_Ind_MalyKomin : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба вертикальная";
};
class Land_Ind_Mlyn_01 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод 1";
};
class Land_Ind_Mlyn_02 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод 2";
};
class Land_Ind_Mlyn_03 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод 3";
};
class Land_Ind_Mlyn_04 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод 4";
};
class Land_Ind_Mlyn_D1 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод D1";
};
class Land_Ind_Mlyn_D2 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод D2";
};
class Land_Ind_Oil_Pump_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Нефтенной насос";
};
class Land_Ind_Oil_Tower_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Нефтенная буровая";
};
class Land_Ind_Pec_01 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод V1";
};
class Land_Ind_Pec_02 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод V2";
};
class Land_Ind_Pec_03 : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод V3";
};
class Land_Ind_Pec_03a : Land_Ind_Pec_03 {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод V4";
};
class Land_Ind_Pec_03b : Land_Ind_Pec_03 {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод V5";
};

class Land_IndPipe1_stair : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (перход)";
};
class Land_IndPipe1_stair_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба (перход)";
};
class Land_IndPipe1_stair_todo_delete : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (перход)";
};
class Land_IndPipe2_big_18 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх)";
};
class Land_IndPipe2_big_18ladder : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница, длинная)";
};
class Land_IndPipe2_big_18ladder_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница, длинная)";
};
class Land_IndPipe2_big_9 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница, короткая)";
};
class Land_IndPipe2_big_ground1 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница)";
};
class Land_IndPipe2_big_ground1_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница)";
};
class Land_IndPipe2_big_ground2 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница)";
};
class Land_IndPipe2_big_ground2_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба (верх, лестница)";
};
class Land_IndPipe2_big_support : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (опора)";
};
class Land_IndPipe2_bigBuild1_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild1_L_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild1_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild1_R_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild2_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild2_L_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild2_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigBuild2_R_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, средняя, перекрещивание)";
};
class Land_IndPipe2_bigL_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, угол)";
};
class Land_IndPipe2_bigL_L_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (лестицы, угол)";
};
class Land_IndPipe2_bigL_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, угол)";
};
class Land_IndPipe2_bigL_R_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (лестицы, угол)";
};
class Land_IndPipe2_Small_9 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба (small)";
};
class Land_IndPipe2_Small_ground1 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L (small, ground)";
};
class Land_IndPipe2_Small_ground2 : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба R (small, ground)";
};
class Land_IndPipe2_SmallBuild1_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба B1 L (small)";
};
class Land_IndPipe2_SmallBuild1_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба B1 R (small)";
};
class Land_IndPipe2_SmallBuild2_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба B2 L (small)";
};
class Land_IndPipe2_SmallBuild2_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба B2 R (small)";
};
class Land_IndPipe2_SmallL_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L1 L (small)";
};
class Land_IndPipe2_SmallL_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба L1 R (small)";
};
class Land_IndPipe2_T_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба T R (small)";
};
class Land_IndPipe2_T_R : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба T L (small)";
};
class Land_komin : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба вертикальная (большая)";
};
class Land_Ind_Quarry : House {
    textPlural = "Здания (Завод)";
    displayName = "Завод (высокий, старый)";
};
class Land_Ind_SawMill : House {
    textPlural = "Здания (Завод)";
    displayName = "Нефтеперерабатывающий завод (small, песок)";
};
class Land_Ind_SawMillPen : House {
    textPlural = "Здания (Завод)";
    displayName = "Навес (песок)";
};

class Land_Ind_Shed_01_end : House {
    textPlural = "Здания (Завод)";
    displayName = "Навес (большой, арка)";
};
class Land_Ind_Shed_01_EP1 : House_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Навес (большой)";
};
class Land_Ind_Shed_01_main : House {
    textPlural = "Здания (Завод)";
    displayName = "Навес (большой, крыша)";
};
class Land_Ind_Shed_02_end : House {
    textPlural = "Здания (Завод)";
    displayName = "Навес (арка)";
};
class Land_Ind_Shed_02_EP1 : Land_Ind_Shed_01_EP1 {
    textPlural = "Здания (Завод)";
    displayName = "Навес (крыша)";
};
class Land_Ind_Shed_02_main : House {
    textPlural = "Здания (Завод)";
    displayName = "Навес (крыша)";
};


class Land_ind_silomale : House {
    textPlural = "Здания (Завод)";
    displayName = "Винный завод (хранилище)";
};

class Land_Ind_SiloVelke_01 : House {
    textPlural = "Здания (Завод)";
    displayName = "Винный завод (здание)";
};
class Land_Ind_SiloVelke_02 : House {
    textPlural = "Здания (Завод)";
    displayName = "Винный завод (хранилище 2)";
};
class Land_Ind_SiloVelke_most : House {
    textPlural = "Здания (Завод)";
    displayName = "Винный завод (переход)";
};

class Land_Ind_Stack_Big : House {
    textPlural = "Здания (Завод)";
    displayName = "Труба вертикальная (большая)";
};

class Land_Ind_TankBig : House {
    textPlural = "Здания (Завод)";
    displayName = "Хранилище (большое)";
};
class Land_Ind_Vysypka : House {
    textPlural = "Здания (Завод)";
    displayName = "Сортировачный пунк";
};
class Land_Ind_Workshop01_01 : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад (мал)";
};
class Land_Ind_Workshop01_02 : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад 2 (мал)";
};
class Land_Ind_Workshop01_03 : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад 3 (мал)";
};
class Land_Ind_Workshop01_04 : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад 4 (мал)";
};
class Land_Ind_Workshop01_box : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад 4 (боковина)";
};
class Land_Ind_Workshop01_L : House {
    textPlural = "Здания (Завод)";
    displayName = "Склад 4 (угол)";
};



//----------------------------------------------
//				Здания (Алтис)
//----------------------------------------------
class Land_spp_Tower_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_Hospital_side1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_Hospital_side2_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_Hospital_main_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_03_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_wpp_Turbine_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_wpp_Turbine_V2_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Garage_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_Garage_V2_F : Land_i_Garage_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_Calvary_02_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_GH_House_1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Addon_04_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_Addon_03mid_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_Addon_03_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_dp_smallFactory_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Shop_01_V2_F : Land_i_Shop_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_Shop_01_V1_F : Land_i_Shop_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Shop_01_V3_F : Land_i_Shop_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_Shop_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_02_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_02_V2_F : Land_i_House_Big_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_House_Big_02_V1_F : Land_i_House_Big_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_02_V3_F : Land_i_House_Big_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_House_Big_02_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_01_V2_F : Land_i_House_Small_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_House_Small_01_V1_F : Land_i_House_Small_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_01_V3_F : Land_i_House_Small_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_02_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_u_House_Small_02_V1_F : Land_i_House_Small_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_02_V2_F : Land_i_House_Small_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Small_02_V3_F : Land_i_House_Small_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_House_Small_02_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_01_V2_F : Land_i_House_Big_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_House_Big_01_V1_F : Land_i_House_Big_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_House_Big_01_V3_F : Land_i_House_Big_01_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_House_Big_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_House_Small_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_BellTower_01_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_BellTower_01_V2_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_Shop_02_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Shop_02_V2_F : Land_i_Shop_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_Shop_02_V1_F : Land_i_Shop_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_i_Shop_02_V3_F : Land_i_Shop_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_d_Shop_02_V1_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_Addon_02_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_d_Addon_02_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_i_Addon_02_V1_F : Land_u_Addon_02_V1_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_Gazebo_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_House_2_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_MainBuilding_entry_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_MainBuilding_left_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_MainBuilding_right_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_GH_MainBuilding_middle_F : House_F {
    textPlural = "Здания (Алтис)";
};
class Land_u_Addon_01_V1_F : House_Small_f {
    textPlural = "Здания (Алтис)";
};
class Land_Offices_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";  
};
class Land_i_Shop_01_V1_F : House_F {
    textPlural = "Здания (Алтис)";  
};
class Land_Kiosk_blueking_F : House_Small_f {
    textPlural = "Здания (Алтис)";  
};
class Land_Kiosk_gyros_F : Land_Kiosk_blueking_F {
    textPlural = "Здания (Алтис)";  
};
class Land_Kiosk_redburger_F : Land_Kiosk_blueking_F {
    textPlural = "Здания (Алтис)";  
};
class Land_Kiosk_papers_F : Land_Kiosk_blueking_F {
    textPlural = "Здания (Алтис)";  
};


//----------------------------------------------
//              ВВП
//----------------------------------------------
class Land_LandMark_F : House_F {
    textPlural = "ВВП";
};
class Land_ReservoirTank_Airport_F : House_F {
    textPlural = "ВВП";
};
class Land_Airport_center_F : House_F {
    textPlural = "ВВП";
};
class Land_Airport_left_F : Land_Airport_center_F {
    textPlural = "ВВП";
};
class Land_Airport_right_F : Land_Airport_left_F {
    textPlural = "ВВП";
};
class Land_Mil_ControlTower : House {
    textPlural = "ВВП";
    displayName = "Контрольная башня";
};
class Land_Mil_ControlTower_dam : Land_Mil_ControlTower {
    textPlural = "ВВП";
    displayName = "Контрольная башня (урон)";
};
class Land_Mil_ControlTower_EP1 : House_EP1 {
    textPlural = "ВВП";
    displayName = "Контрольная башня";
};
class Land_Mil_ControlTower_dam_EP1 : Land_Mil_ControlTower_EP1 {
    textPlural = "ВВП";
    displayName = "Контрольная башня (урон)";
};
class Land_Mil_hangar_EP1 : House_EP1 {
    textPlural = "ВВП";
    displayName = "Ангар (песок)";
};
class Land_Airport_Tower_F : House_F {
    textPlural = "ВВП";
};




//----------------------------------------------
//				Здания (Военные)
//----------------------------------------------
class Land_TentHangar_V1_F : House_F {
    textPlural = "Здания (Военные)";
};
class Bunker_PMC : House {
    textPlural = "Здания (Военные)";
};
class Land_Bunker_F : House_F {
    textPlural = "Здания (Военные)";
};
class Land_MilOffices_V1_F : House_F {
    textPlural = "Здания (Военные)";
};
class Land_Hangar_F : House_F {
    textPlural = "Здания (Военные)";
};
class WaterBasin_conc_EP1 : House {
    textPlural = "Здания (Военные)";
    displayName = "Водная преграда";
};
class WarfareBDepot : WarfareBBaseStructure {
    textPlural = "Здания (Военные)";
};
class WarfareBAirport : WarfareBBaseStructure {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_HQ_V1_F : Cargo_HQ_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_HQ_V2_F : Cargo_HQ_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_Patrol_V3_F : Cargo_Patrol_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_Patrol_V1_F : Cargo_Patrol_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_Patrol_V2_F : Cargo_Patrol_base_F {
    textPlural = "Здания (Военные)";
};
class Land_i_Barracks_V1_F : House_F {
    textPlural = "Здания (Военные)";
};

class Land_i_Barracks_V2_F : Land_i_Barracks_V1_F {
    textPlural = "Здания (Военные)";
};
class Land_u_Barracks_V2_F : Land_i_Barracks_V1_F {
    textPlural = "Здания (Военные)";
};
class Land_Cargo_HQ_V3_F : Cargo_HQ_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Research_HQ_F : House_F {
    textPlural = "Здания (Военные)";
};
class Land_Medevac_house_V1_F : Cargo_House_base_F {
    textPlural = "Здания (Военные)";
};
class Land_Radar_Small_F : House_Small_f {
    textPlural = "Здания (Военные)";
    displayName = "Радар (мал)";
};
class Land_Radar_F : House_F {
    textPlural = "Здания (Военные)";
    displayName = "Радар";
};
class Land_Mil_Barracks : House {
    textPlural = "Здания (Военные)";
    displayName = "барак 1";
};
class Land_Mil_Barracks_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "барак 2";
};
class Land_Mil_Barracks_i : House {
    textPlural = "Здания (Военные)";
    displayName = "барак 3";
};
class Land_Mil_Barracks_i_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "барак 4";
};
class Land_Mil_Barracks_L : House {
    textPlural = "Здания (Военные)";
    displayName = "барак 5";
};
class Land_Mil_Barracks_L_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "барак 5";
};
class Land_Mil_Guardhouse : House {
    textPlural = "Здания (Военные)";
    displayName = "КПП";
};
class Land_Mil_Guardhouse_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "КПП (песок)";
};
class Land_Mil_House : House {
    textPlural = "Здания (Военные)";
    displayName = "Дом";
};
class Land_Mil_House_dam : Land_Mil_House {
    textPlural = "Здания (Военные)";
    displayName = "Дом (урон)";
};
class Land_Mil_House_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "Дом";
};
class Land_Mil_House_dam_EP1 : Land_Mil_House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "Дом (урон)";
};

class Land_Mil_Repair_center_EP1 : House_EP1 {
    textPlural = "Здания (Военные)";
    displayName = "Мастерская";
};
class Land_Dome_Big_F : House_F {
    textPlural = "Здания (Военные)";
};
class Land_Dome_Small_F : House_F {
    textPlural = "Здания (Военные)";
};



//----------------------------------------------
//				Здания (Такистат)
//----------------------------------------------
class Land_A_Minaret_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Минарет (руины)";
};
class Land_A_Minaret_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Минарет";
};
class Land_A_Minaret_Porto_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Минарет (камень)";
};
class Land_A_Minaret_porto_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Минарет (камень, руины)";
};
class Land_A_Mosque_big_addon_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (боковина)";
};
class Land_A_Mosque_big_hq_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (центер)";
};
class Land_A_Mosque_big_minaret_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (минерат)";
};
class Land_A_Mosque_big_minaret_1_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (минерат, руины)";
};
class Land_A_Mosque_big_minaret_2_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (минерат 2, руины)";
};
class Land_A_Mosque_big_minaret_2_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (минерат 2)";
};
class Land_A_Mosque_big_wall_corner_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (стена, угол)";
};
class Land_A_Mosque_big_wall_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (стена)";
};
class Land_A_Mosque_big_wall_gate_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (врата, руины)";
};
class Land_A_Mosque_big_wall_gate_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (врата)";
};
class Land_A_Mosque_small_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (Деревня)";
};
class Land_A_Mosque_small_1_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (Деревня, руины)";
};
class Land_A_Mosque_small_2_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (Деревня 2)";
};
class Land_A_Mosque_small_2_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Мечеть (Деревня 2, руины)";
};
class Land_A_Villa_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Вилла";
};
class Land_A_Villa_dam_EP1 : Land_A_Villa_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Вилла (руины)";
};
class Land_A_CityGate1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Городские ворота";
};
class Land_House_C_10_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 10 (руины)";
};
class Land_House_C_10_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 10";
};
class Land_House_C_11_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 11 (руины)";
};
class Land_House_C_11_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 11";
};
class Land_House_C_12_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 12 (руины)";
};
class Land_House_C_12_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 12";
};
class Land_House_C_1_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 1 (руины)";
};
class Land_House_C_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 1";
};
class Land_House_C_1_v2_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 1-а (руины)";
};
class Land_House_C_1_v2_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 1-а";
};
class Land_House_C_2_DAM_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 2 (руины)";
};
class Land_House_C_2_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 2";
};
class Land_House_C_3_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 3 (руины)";
};
class Land_House_C_3_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 3";
};
class Land_House_C_4_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 4 (руины)";
};
class Land_House_C_4_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 4";
};
class Land_House_C_5_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5 (руины)";
};
class Land_House_C_5_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5";
};
class Land_House_C_5_V1_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-1 (руины)";
};
class Land_House_C_5_V1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-1";
};
class Land_House_C_5_V2_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-2 (руины)";
};
class Land_House_C_5_V2_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-2";
};
class Land_House_C_5_V3_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-3 (руины)";
};
class Land_House_C_5_V3_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 5-3";
};
class Land_House_C_9_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 9 (руины)";
};
class Land_House_C_9_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом С 9";
};

class Land_House_K_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 1";
};
class Land_House_K_3_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 3 (руины)";
};
class Land_House_K_3_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 3";
};
class Land_House_K_5_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 5 (руины)";
};
class Land_House_K_5_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 5";
};
class Land_House_K_6_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 6 (руины)";
};
class Land_House_K_6_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 6";
};
class Land_House_K_7_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 7 (руины)";
};
class Land_House_K_7_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 7";
};
class Land_House_K_8_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 8 (руины)";
};
class Land_House_K_8_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом К 8";
};

class Land_House_L_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 1";
};
class Land_House_L_3_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 3 (руины)";
};
class Land_House_L_3_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 3";
};
class Land_House_L_4_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 4 (руины)";
};
class Land_House_L_4_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 4";
};
class Land_House_L_6_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 6 (руины)";
};
class Land_House_L_6_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 6";
};
class Land_House_L_7_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 7 (руины)";
};
class Land_House_L_7_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 7";
};
class Land_House_L_8_dam_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 8 (руины)";
};
class Land_House_L_8_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 8";
};
class Land_House_L_9_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "Дом L 9";
};
class Land_Terrace_K_1_EP1 : House_EP1 {
    textPlural = "Здания (Такистат)";
    displayName = "терраса";
};



//----------------------------------------------
//				Бункер
//----------------------------------------------
class Land_BagBunker_Tower_F : BagBunker_base_F {
    textPlural = "Бункер";
};
class Land_BagBunker_Large_F : BagBunker_base_F {
    textPlural = "Бункер";
};
class Land_BagBunker_Small_F : BagBunker_base_F {
    textPlural = "Бункер";
};
class Land_HBarrierTower_F : HBarrier_base_F {
    textPlural = "Бункер";
};
class WarfareBCamp : Fortress1 {
    textPlural = "Бункер";
    displayName = "Песчанный бункер 2";
};
class Fort_CAmp : WarfareBCamp {
    textPlural = "Бункер";
    displayName = "Песчанный бункер";
};
class Land_Research_house_V1_F : House_Small_F {
    textPlural = "Бункер";
};
class Land_Cargo_House_V1_F : Cargo_House_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_House_V3_F : Cargo_House_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_House_V2_F : Cargo_House_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_F : Cargo_Tower_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V3_F : Cargo_Tower_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V2_F : Cargo_Tower_base_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No1_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No2_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No3_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No4_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No5_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Cargo_Tower_V1_No6_F : Land_Cargo_Tower_V1_F {
    textPlural = "Бункер";
};
class Land_Medevac_HQ_V1_F : Cargo_HQ_base_F {
    textPlural = "Бункер";
};
class Land_vez : House {
    textPlural = "Бункер";
    displayName = "Наблюдательная вышка";
};
class Fort_Nest : Land_fortified_nest_small {
    textPlural = "Бункер";
    displayName = "Бункер из песка (зеленый)";
};
class Fort_Barracks_USMC : WarfareBBaseStructure {
    textPlural = "Бункер";
    displayName = "Бункер - казарма";
};
class Base_WarfareBBarracks : WarfareBBaseStructure {
    textPlural = "Бункер";
    displayName = "Бункер - штаб";
};





//----------------------------------------------
//				Лестница
//----------------------------------------------
class ladder : House {
    textPlural = "Лестница";
    displayName = "Лестница вертикальня";
};
class LadderLong : House {
    textPlural = "Лестница";
    displayName = "Лестница вертикальня длинная";
};
class Land_WoodenRamp : House {
    textPlural = "Лестница";
    displayName = "Деревянная рампа";
};
class FootBridge_0_ACR : FootBridge_Base_ACR {
    textPlural = "Лестница";
    displayName = "Деревянный мост 0(деревянный)";
};
class FootBridge_30_ACR : FootBridge_Base_ACR {
    textPlural = "Лестница";
    displayName = "Деревянная рампа (большая) 30 ";
};
class Land_ladder : House {
    textPlural = "Лестница";
    displayName = "Лестница вертикальня";
};
class Land_ladder_half : House {
    textPlural = "Лестница";
    displayName = "Лестница вертикальня";
};
class Land_ladder_half_EP1 : Land_ladder_half {
    textPlural = "Лестница";
    displayName = "Лестница вертикальня";
};


//----------------------------------------------
//             Аэропорт
//----------------------------------------------




//----------------------------------------------
//              Здания (Алтис, деревнья)
//----------------------------------------------
class Land_d_Windmill01_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Windmill01_F : Land_d_Windmill01_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_Slum_House03_F : Land_Slum_House01_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_Slum_House01_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_Slum_House02_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_cargo_addon02_V2_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_Metal_Shed_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseSmall_V2_F : Land_i_Stone_HouseSmall_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseSmall_V2_F : Land_i_Stone_HouseSmall_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseBig_V3_F : Land_i_Stone_HouseBig_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_d_Stone_HouseBig_V1_F : House_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseBig_V1_F : House_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseSmall_V3_F : Land_i_Stone_HouseSmall_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_d_Stone_Shed_V1_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_Shed_V1_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_Shed_V2_F : Land_i_Stone_Shed_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_Shed_V3_F : Land_i_Stone_Shed_V1_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_d_Stone_HouseSmall_V1_F : House_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_i_Stone_HouseSmall_V1_F : House_F {
    textPlural = "Здания (Алтис, деревнья)";
};
class Land_cargo_house_slum_F : House_Small_f {
    textPlural = "Здания (Алтис, деревнья)";
};



//----------------------------------------------
//             USMC
//----------------------------------------------
class Base_WarfareBAircraftFactory;
class BASE_WarfareBAntiAirRadar;
class BASE_WarfareBArtilleryRadar;
class Base_WarfareBBarracks;
class Base_WarfareBBarrier10x;
class Base_WarfareBBarrier10xTall;
class Base_WarfareBBarrier5x;
class Base_WarfareBHeavyFactory;
class Base_WarfareBLightFactory;
class Base_WarfareBUAVterminal;
class Base_WarfareBVehicleServicePoint;

class USMC_WarfareBAircraftFactory : Base_WarfareBAircraftFactory {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBAntiAirRadar : BASE_WarfareBAntiAirRadar {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBBarracks : Base_WarfareBBarracks {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBBarrier10x : Base_WarfareBBarrier10x {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBBarrier10xTall : Base_WarfareBBarrier10xTall {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBBarrier5x : Base_WarfareBBarrier5x {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBLightFactory : Base_WarfareBLightFactory {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBUAVterminal : Base_WarfareBUAVterminal {
    textPlural = "USMC";
    displayName = "";
};
class USMC_WarfareBVehicleServicePoint : Base_WarfareBVehicleServicePoint {
    textPlural = "USMC";
    displayName = "";
};


//----------------------------------------------
//             TK
//----------------------------------------------
class TK_GUE_WarfareBAircraftFactory_Base_EP1;
class TK_GUE_WarfareBAntiAirRadar_Base_EP1;
class TK_GUE_WarfareBArtilleryRadar_Base_EP1;
class TK_GUE_WarfareBBarracks_Base_EP1;
class TK_GUE_WarfareBLightFactory_base_EP1;
class TK_GUE_WarfareBUAVterminal_Base_EP1;
class TK_GUE_WarfareBVehicleServicePoint_Base_EP1;
class TK_WarfareBAircraftFactory_Base_EP1;
class TK_WarfareBAntiAirRadar_Base_EP1;
class TK_WarfareBArtilleryRadar_Base_EP1;
class TK_WarfareBBarracks_Base_EP1;
class TK_WarfareBHeavyFactory_Base_EP1;
class TK_WarfareBLightFactory_base_EP1;
class TK_WarfareBUAVterminal_Base_EP1;
class TK_WarfareBVehicleServicePoint_Base_EP1;


class TK_GUE_WarfareBAircraftFactory_EP1 : TK_GUE_WarfareBAircraftFactory_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBAntiAirRadar_EP1 : TK_GUE_WarfareBAntiAirRadar_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBArtilleryRadar_EP1 : TK_GUE_WarfareBArtilleryRadar_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBBarracks_EP1 : TK_GUE_WarfareBBarracks_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBBarrier10x_EP1 : Base_WarfareBBarrier10x {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBBarrier10xTall_EP1 : Base_WarfareBBarrier10xTall {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBBarrier5x_EP1 : Base_WarfareBBarrier5x {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBHeavyFactory_Base_EP1 : Base_WarfareBHeavyFactory {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBHeavyFactory_EP1 : TK_GUE_WarfareBHeavyFactory_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBLightFactory_EP1 : TK_GUE_WarfareBLightFactory_base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBUAVterminal_EP1 : TK_GUE_WarfareBUAVterminal_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_GUE_WarfareBVehicleServicePoint_EP1 : TK_GUE_WarfareBVehicleServicePoint_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBAircraftFactory_EP1 : TK_WarfareBAircraftFactory_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBAntiAirRadar_EP1 : TK_WarfareBAntiAirRadar_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBArtilleryRadar_EP1 : TK_WarfareBArtilleryRadar_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBBarracks_EP1 : TK_WarfareBBarracks_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBBarrier10x_EP1 : Base_WarfareBBarrier10x {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBBarrier10xTall_EP1 : Base_WarfareBBarrier10xTall {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBBarrier5x_EP1 : Base_WarfareBBarrier5x {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBHeavyFactory_EP1 : TK_WarfareBHeavyFactory_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBLightFactory_EP1 : TK_WarfareBLightFactory_base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBUAVterminal_EP1 : TK_WarfareBUAVterminal_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};
class TK_WarfareBVehicleServicePoint_EP1 : TK_WarfareBVehicleServicePoint_Base_EP1 {
    textPlural = "TK";
    displayName = "";
};


//----------------------------------------------
//             RU
//----------------------------------------------
class RU_WarfareBAircraftFactory : Base_WarfareBAircraftFactory {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBAntiAirRadar : BASE_WarfareBAntiAirRadar {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBBarracks : Base_WarfareBBarracks {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBBarrier10x : Base_WarfareBBarrier10x {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBBarrier10xTall : Base_WarfareBBarrier10xTall {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBBarrier5x : Base_WarfareBBarrier5x {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBLightFactory : Base_WarfareBLightFactory {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBUAVterminal : Base_WarfareBUAVterminal {
    textPlural = "RU";
    displayName = "";
};
class RU_WarfareBVehicleServicePoint : Base_WarfareBVehicleServicePoint {
    textPlural = "RU";
    displayName = "";
};

//----------------------------------------------
//             US
//----------------------------------------------
class US_WarfareBAircraftFactory_Base_EP1;
class US_WarfareBAntiAirRadar_Base_EP1;
class US_WarfareBArtilleryRadar_Base_EP1;
class US_WarfareBHeavyFactory_Base_EP1;
class US_WarfareBLightFactory_base_EP1;
class US_WarfareBUAVterminal_Base_EP1;
class US_WarfareBVehicleServicePoint_Base_EP1;
class US_WarfareBBarracks_Base_EP1;

class US_WarfareBAircraftFactory_EP1 : US_WarfareBAircraftFactory_Base_EP1 {
    textPlural = "US";
    displayName = "";
};

class US_WarfareBAntiAirRadar_EP1 : US_WarfareBAntiAirRadar_Base_EP1 {
    textPlural = "US";
    displayName = "";
};

class US_WarfareBArtilleryRadar_EP1 : US_WarfareBArtilleryRadar_Base_EP1 {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBBarracks_EP1 : US_WarfareBBarracks_Base_EP1 {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBBarrier10x_EP1 : Base_WarfareBBarrier10x {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBBarrier10xTall_EP1 : Base_WarfareBBarrier10xTall {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBBarrier5x_EP1 : Base_WarfareBBarrier5x {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBHeavyFactory_EP1 : US_WarfareBHeavyFactory_Base_EP1 {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBLightFactory_EP1 : US_WarfareBLightFactory_base_EP1 {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBUAVterminal_EP1 : US_WarfareBUAVterminal_Base_EP1 {
    textPlural = "US";
    displayName = "";
};
class US_WarfareBVehicleServicePoint_EP1 : US_WarfareBVehicleServicePoint_Base_EP1 {
    textPlural = "US";
    displayName = "";
};
class WarfareBAircraftFactory_CDF : Base_WarfareBAircraftFactory {
    textPlural = "US";
    displayName = "";
};
class WarfareBAircraftFactory_Gue : Base_WarfareBAircraftFactory {
    textPlural = "US";
    displayName = "";
};
class WarfareBAircraftFactory_Ins : Base_WarfareBAircraftFactory {
    textPlural = "US";
    displayName = "";
};



//----------------------------------------------
//             GUE
//----------------------------------------------
class GUE_WarfareBAircraftFactory : Base_WarfareBAircraftFactory {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBAntiAirRadar : BASE_WarfareBAntiAirRadar {
    textPlural = "GUE";
    displayName = "";
};
class Gue_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "GUE";
    displayName = "";
};
class Gue_WarfareBBarracks : Base_WarfareBBarracks {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBBarrier10x : Base_WarfareBBarrier10x {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBBarrier10xTall : Base_WarfareBBarrier10xTall {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBBarrier5x : Base_WarfareBBarrier5x {
    textPlural = "GUE";
    displayName = "";
};
class Gue_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "GUE";
    displayName = "";
};
class Gue_WarfareBLightFactory : Base_WarfareBLightFactory {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBUAVterminal : Base_WarfareBUAVterminal {
    textPlural = "GUE";
    displayName = "";
};
class GUE_WarfareBVehicleServicePoint : Base_WarfareBVehicleServicePoint {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBAircraftFactory : Base_WarfareBAircraftFactory {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBAntiAirRadar : BASE_WarfareBAntiAirRadar {
    textPlural = "GUE";
    displayName = "";
};
class Ins_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "GUE";
    displayName = "";
};
class Ins_WarfareBBarracks : Base_WarfareBBarracks {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBBarrier10x : Base_WarfareBBarrier10x {
    textPlural = "GUE";
    displayName = "";
};
    class INS_WarfareBBarrier10xTall : Base_WarfareBBarrier10xTall {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBBarrier5x : Base_WarfareBBarrier5x {
    textPlural = "GUE";
    displayName = "";
};
class Ins_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "GUE";
    displayName = "";
};
class Ins_WarfareBLightFactory : Base_WarfareBLightFactory {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBUAVterminal : Base_WarfareBUAVterminal {
    textPlural = "GUE";
    displayName = "";
};
class INS_WarfareBVehicleServicePoint : Base_WarfareBVehicleServicePoint {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBAircraftFactory : Base_WarfareBAircraftFactory {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBAntiAirRadar : BASE_WarfareBAntiAirRadar {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBArtilleryRadar : BASE_WarfareBArtilleryRadar {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBBarracks : Base_WarfareBBarracks {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBBarrier10x : Base_WarfareBBarrier10x {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBBarrier10xTall : Base_WarfareBBarrier10xTall {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBBarrier5x : Base_WarfareBBarrier5x {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBHeavyFactory : Base_WarfareBHeavyFactory {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBLightFactory : Base_WarfareBLightFactory {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBUAVterminal : Base_WarfareBUAVterminal {
    textPlural = "GUE";
    displayName = "";
};
class CDF_WarfareBVehicleServicePoint : Base_WarfareBVehicleServicePoint {
    textPlural = "GUE";
    displayName = "";
};














};


